...
# A spherical-quality view must show the complete segmented target rather than
# merely contain some foreground.  A scale-relative margin keeps the contract
# meaningful for both real images and small CPU fixtures.
SPHERICAL_MIN_MASK_BORDER_MARGIN_RATIO = 0.02
...
def object_spherical_farthest_frame_indices(
    ...
    *,
    ...
    threshold = float(alpha_threshold) * 255.0
    foreground_area: dict[int, int] = {}
    foreground_border_margin_pixels: dict[int, int] = {}
    foreground_border_margin_required_pixels: dict[int, int] = {}
    border_rejected: list[int] = []
    foreground_valid = []
    for index, name in enumerate(names):
        path = Path(masks_dir) / name
        if not path.is_file():
        ...
        border_margin = int(
            min(
                int(x.min()),
                int(mask.shape[1] - 1 - x.max()),
                int(y.min()),
                int(mask.shape[0] - 1 - y.max()),
            )
        )
        required_margin = max(
            1,
            int(
                math.ceil(
                    SPHERICAL_MIN_MASK_BORDER_MARGIN_RATIO
                    * float(min(mask.shape))
                )
            ),
        )
        foreground_border_margin_pixels[index] = border_margin
        foreground_border_margin_required_pixels[index] = required_margin
        if border_margin < required_margin:
            border_rejected.append(index)
            continue
    ...
    def candidate_record(indices: Sequence[int], rank: int) -> dict[str, Any]:
        ...
        return {
            ...
            "selected_foreground_border_margin_pixels_by_frame": {
                names[index]: int(foreground_border_margin_pixels[index])
                for index in indices
            },
            "selected_foreground_border_margin_required_pixels_by_frame": {
                names[index]: int(
                    foreground_border_margin_required_pixels[index]
                )
                for index in indices
            },
            "selected_azimuth_degrees_by_frame": {
    ...
    return selected, {
        ...
        "validity_domain": (
            "post_runtime_undistortion_nonempty_mask_with_complete_visibility_margin"
        ),
        "complete_visibility_policy": (
            "foreground_bbox_margin_ge_2_percent_of_short_image_side"
        ),
        "minimum_foreground_border_margin_ratio": (
            SPHERICAL_MIN_MASK_BORDER_MARGIN_RATIO
        ),
        "border_rejected_frame_count": int(len(border_rejected)),
        "border_rejected_frame_names": [names[index] for index in border_rejected],
        "border_rejected_frame_details": {
            names[index]: {
                "actual_margin_pixels": int(
                    foreground_border_margin_pixels[index]
                ),
                "required_margin_pixels": int(
                    foreground_border_margin_required_pixels[index]
                ),
            }
            for index in border_rejected
        },
        "object_center_source": "coordinatewise_median_sparse_object_points_W",
        "object_center_W": center_w.tolist(),
        "gravity_up_W": gravity.tolist(),
        ...
        "selection_candidate_count": len(selection_candidates),
        "selection_candidates": selection_candidates,
        "foreground_nonempty_frame_count": int(
            len(foreground_valid) + len(border_rejected)
        ),
...
def _pose_mask_single_segment_frame_indices(
    ...
    *,
    ...
    selection_identity: str = "real_capture",
    precomputed_object_center_w: np.ndarray | None = None,
) -> tuple[np.ndarray, dict[str, Any]]:
    ...
    if precomputed_object_center_w is None:
        ...
        center_w = np.asarray(precomputed_object_center_w, dtype=np.float64)
        if center_w.shape != (3,) or not np.isfinite(center_w).all():
            raise ValueError("precomputed all-view object center must be finite [3]")
        ray_stats = None
        center_source = "precomputed_all_view_o_before_selection"
    selector = (
    ...
    record.update(
        {
            ...
            "o_frozen_before_selection": precomputed_object_center_w is not None,
...
def _pose_mask_selected_quality(
    *,
    ...
    if precomputed_object_frame is not None:
        frame_stats = precomputed_object_frame.stats
    else:
        # Backwards-compatible helper path.  The unified manual pipeline always
        # supplies the all-view frame and therefore never re-estimates O here.
        masks = []
    ...
    *,
    ...
    for segment in segments:
        ...
        try:
            _initial_selected_local, selection = _pose_mask_single_segment_frame_indices(
                ...
                precomputed_object_center_w=(
                    None
                    if precomputed_object_frame is None
                    else np.asarray(precomputed_object_frame.T_O2W[:3, 3])
                ),
            )
            ...
            for candidate_rank, candidate_record in enumerate(selection_candidates):
                ...
                quality = _pose_mask_selected_quality(
                    ...
                    precomputed_object_frame=precomputed_object_frame,
                )
    ...
    record = {
        **selection,
        ...
        "o_frozen_before_selection": precomputed_object_frame is not None,
        "selection_reestimated_o": precomputed_object_frame is None,
        "all_view_o_T_O2W_sha256": (
            None
            if precomputed_object_frame is None
            else array_sha256(precomputed_object_frame.T_O2W)
        ),
    }
...
def pose_mask_training_spherical_farthest_frame_indices(
    ...
    *,
    ...
    """Use the official-training single-seed spherical FPS policy.

    This deliberately does not enumerate alternative starts and does not rank
    subsets with real-input quality gates.  When ``object_center_w`` is passed,
    it is the center of the already-frozen all-view model-O frame.
    """
    ...
    center = (
        None
        if object_center_w is None
        else np.asarray(object_center_w, dtype=np.float64)
    )
    ray_stats = None
    if center is None:
        threshold = float(alpha_threshold) * 255.0
    ...
    record.update(
        {
            ...
            "o_frozen_before_selection": object_center_w is not None,
            "selection_reestimated_o": object_center_w is None,
        }
    )
    return selected, record
...
def _all_undistorted_masks(
    frame_names: Sequence[str],
    masks_dir: Path,
    intrinsics: np.ndarray,
    camera_by_name: dict[str, dict[str, Any]],
) -> list[np.ndarray]:
    """Load the complete observable mask domain used to freeze model-O."""

    names = [str(value) for value in frame_names]
    k_all = np.asarray(intrinsics, dtype=np.float64)
    if k_all.shape != (len(names), 3, 3) or not np.isfinite(k_all).all():
        raise ValueError("all-view model-O requires finite K for every input view")
...
def precompute_pose_mask_object_frame(
    *,
    frame_names: Sequence[str],
    masks_dir: Path,
    intrinsics: np.ndarray,
    ...
    """Freeze O from all eligible views before any model-view selection."""

    names = [str(value) for value in frame_names]
    masks = _all_undistorted_masks(
        names,
        masks_dir,
        intrinsics,
        camera_by_name,
    ...
    valid = [
        ...
        if int(np.count_nonzero(np.asarray(mask) >= 128)) >= 4
    ]
    if len(valid) < 2:
        raise InsufficientForegroundViewsError(available=len(valid), required=2)
    reference = max(
        valid,
        key=lambda index: (int(np.count_nonzero(masks[index] >= 128)), -index),
    )
    frame = canonicalize_pose_mask_runtime_object_frame(
    ...
    frame.stats["all_view_o_frozen_before_selection"] = True
    frame.stats["all_input_view_count"] = int(len(names))
    frame.stats["all_input_valid_mask_view_count"] = int(len(valid))
    frame.stats["all_input_frame_names_sha256"] = canonical_json_sha256(names)
    frame.stats["all_view_reference_frame_index"] = int(reference)
    frame.stats["all_view_reference_frame_name"] = names[int(reference)]
    frame.contract["o_estimation_order"] = (
        "all eligible RGB/mask camera views -> freeze model-O -> select model views"
    )
    frame.contract["all_input_view_count"] = int(len(names))
    frame.contract["all_input_frame_names_sha256"] = canonical_json_sha256(names)
    return frame


def random_valid_frame_indices(
    frame_names: Sequence[str],
    masks_dir: Path,
    count: int,
    *,
    ...
    """Select a reproducible random subset after all-view O is frozen."""

    names = [str(value) for value in frame_names]
    masks = _all_undistorted_masks(names, masks_dir, intrinsics, camera_by_name)
    threshold = float(alpha_threshold) * 255.0
    valid = np.asarray(
        [
            index
            for index, mask in enumerate(masks)
            if int(np.count_nonzero(np.asarray(mask) > threshold)) > 0
        ],
        dtype=np.int64,
    )
    requested = int(count)
    if len(valid) < requested:
        raise InsufficientForegroundViewsError(
            available=int(len(valid)), required=requested
        )
    draw = np.random.default_rng(int(random_seed)).choice(
        valid, size=requested, replace=False
    )
    selected = np.sort(np.asarray(draw, dtype=np.int64))
    return selected, {
        "policy": "uniform_random_valid_mask_after_all_view_o_v1",
        "deterministic": True,
        "random_seed": int(random_seed),
        "candidate_frame_count": int(len(names)),
        "foreground_valid_candidate_count": int(len(valid)),
        "raw_draw_order": [int(value) for value in draw],
        "selected_source_view_indices": [int(value) for value in selected],
        "selected_frame_names": [names[int(value)] for value in selected],
        "execution_order": "selected source indices sorted chronologically",
        "o_frozen_before_selection": True,
    }


def fixed_frame_name_indices(
    frame_names: Sequence[str],
    masks_dir: Path,
    count: int,
    *,
    requested_frame_names: Sequence[str],
    alpha_threshold: float,
    intrinsics: np.ndarray,
    camera_by_name: dict[str, dict[str, Any]],
) -> tuple[np.ndarray, dict[str, Any]]:
    """Replay an exact named subset after the all-view object frame is frozen.

    This is intentionally strict: every requested name must occur exactly once,
    the list length must equal ``selected_view_count``, and every selected mask
    must remain nonempty after the same runtime undistortion used by the model.
    """

    names = [str(value) for value in frame_names]
    requested = [str(value) for value in requested_frame_names]
    expected_count = int(count)
    if len(requested) != expected_count:
        raise ValueError(
            "fixed frame-name count differs from selected_view_count: "
            f"names={len(requested)} selected_view_count={expected_count}"
        )
    if len(set(requested)) != len(requested):
        raise ValueError("fixed frame names must be unique")
    duplicate_source_names = sorted(
        name for name in set(names) if names.count(name) != 1
    )
    if duplicate_source_names:
        raise RuntimeError(
            f"source raw cache contains duplicate frame names: {duplicate_source_names}"
        )
    normalized_index = {name: index for index, name in enumerate(names)}
    source_aliases: dict[str, list[int]] = {}
    for index, normalized_name in enumerate(names):
        camera = camera_by_name.get(normalized_name)
        if camera is None:
            raise RuntimeError(f"missing camera metadata for frame={normalized_name}")
        source_name = str(camera.get("source_frame_name") or normalized_name)
        source_aliases.setdefault(source_name, []).append(index)

    resolved_indices: list[int] = []
    missing: list[str] = []
    ambiguous: dict[str, list[int]] = {}
    for requested_name in requested:
        matches = set(source_aliases.get(requested_name, []))
        if requested_name in normalized_index:
            matches.add(normalized_index[requested_name])
        if not matches:
            missing.append(requested_name)
        elif len(matches) != 1:
            ambiguous[requested_name] = sorted(matches)
        else:
            resolved_indices.append(matches.pop())
    if missing:
        raise ValueError(f"fixed frame names are absent from raw cache: {missing}")
    if ambiguous:
        raise ValueError(f"fixed frame names are ambiguous: {ambiguous}")
    if len(set(resolved_indices)) != len(resolved_indices):
        raise ValueError("fixed frame aliases resolve to duplicate source views")
    masks = _all_undistorted_masks(names, masks_dir, intrinsics, camera_by_name)
    threshold = float(alpha_threshold) * 255.0
    empty = [
        requested_name
        for requested_name, index in zip(requested, resolved_indices)
        if int(np.count_nonzero(masks[index] > threshold)) == 0
    ]
    if empty:
        raise InsufficientForegroundViewsError(
            available=expected_count - len(empty), required=expected_count
        )
    selected = np.asarray(resolved_indices, dtype=np.int64)
    selected_normalized_names = [names[int(index)] for index in selected]
    selected_source_names = [
        str(camera_by_name[names[int(index)]].get("source_frame_name") or names[int(index)])
        for index in selected
    ]
    return selected, {
        "policy": "fixed_frame_names_valid_mask_after_all_view_o_v1",
        "deterministic": True,
        "requested_frame_names": requested,
        "selected_source_view_indices": selected.astype(int).tolist(),
        "selected_frame_names": selected_normalized_names,
        "selected_source_frame_names": selected_source_names,
        "candidate_frame_count": int(len(names)),
        "validity_domain": "post_runtime_undistortion_mask",
        "execution_order": "exact requested frame-name order",
        "o_frozen_before_selection": True,
        "fallback_used": False,
        "replacements": [],
    }
...
def build_object_runtime_input(
    ...
    *,
    ...
    fixed_frame_names: Sequence[str] | None = None,
    ...
    all_view_object_frame: RuntimeObjectFrame | None = None
    try:
        with np.load(source_cache, allow_pickle=False) as source:
            ...
            missing_all_camera_rows = [
                name for name in frame_names_all if name not in camera_by_name
            ]
            if missing_all_camera_rows:
                raise RuntimeError(
                    "missing all-view camera metadata for frames="
                    f"{missing_all_camera_rows}"
                )
            if geometry_mode == "pose_mask":
                if not isinstance(frame_config, PoseMaskObjectFrameConfig):
                    raise TypeError(
                        "pose-mask geometry requires PoseMaskObjectFrameConfig"
                    )
                all_view_object_frame = precompute_pose_mask_object_frame(
            ...
            if view_selection_policy == "fixed_frame_names_valid_mask":
                selected_indices, selection_record = fixed_frame_name_indices(
                    frame_names_all,
                    Path(row["masks_dir"]),
                    int(selected_view_count),
                    requested_frame_names=list(fixed_frame_names or []),
                    alpha_threshold=float(alpha_threshold),
                    intrinsics=K_all,
                    camera_by_name=camera_by_name,
                )
            elif fixed_frame_names:
                raise ValueError(
                    "fixed_frame_names may only be supplied with "
                    "view_selection_policy=fixed_frame_names_valid_mask"
                )
            elif view_selection_policy == "lexical_even":
                ...
                if geometry_mode == "pose_mask":
                    if all_view_object_frame is None:
                        raise RuntimeError(
                            "training spherical FPS requires a frozen all-view model-O"
                        )
                    selected_indices, selection_record = (
                        pose_mask_training_spherical_farthest_frame_indices(
                            ...
                            object_center_w=np.asarray(
                                all_view_object_frame.T_O2W[:3, 3],
                                dtype=np.float64,
                            ),
                        )
                    )
                    selection_record.update(
                        {
                            "all_view_o_T_O2W_sha256": array_sha256(
                                all_view_object_frame.T_O2W
                            ),
                ...
                if view_selection_policy == "time_uniform_valid_mask":
                    selection_record.update(
                        {
                            "policy": "time_axis_uniform_valid_mask_after_all_view_o_v1",
                            "o_frozen_before_selection": True,
                        }
                    )
            elif view_selection_policy == "random_valid_mask":
                selected_indices, selection_record = random_valid_frame_indices(
                    frame_names_all,
                    Path(row["masks_dir"]),
                    int(selected_view_count),
                    alpha_threshold=float(alpha_threshold),
                    intrinsics=K_all,
                    camera_by_name=camera_by_name,
                    random_seed=int(selection_seed),
                )
            elif view_selection_policy in {
                ...
                if geometry_mode == "point_mask":
                    ...
                    selector_kwargs["precomputed_object_frame"] = (
                        all_view_object_frame
                    )
                if spherical:
            ...
            source_frame_names_all = (
                [str(value) for value in source["source_frame_name"].tolist()]
                if "source_frame_name" in source.files
                else list(frame_names_all)
            )
            if len(source_frame_names_all) != len(frame_names_all):
                raise RuntimeError("raw-cache source-frame-name binding is malformed")
            selection_record["selected_source_view_indices"] = [
                int(index) for index in selected_indices
            ]
            selection_record["selected_source_frame_names"] = [
                source_frame_names_all[int(index)] for index in selected_indices
            ]
            selection_record["selected_runtime_frame_names"] = [
                frame_names_all[int(index)] for index in selected_indices
            ]
    ...
    selected_reference_view_index = max(
        range(len(masks)),
        key=lambda index: (
            int(np.count_nonzero(np.asarray(masks[index], dtype=np.uint8) >= 128)),
            -index,
        ),
    )
    try:
        ...
        if geometry_mode == "pose_mask":
            if all_view_object_frame is None:
                raise AssertionError("all-view pose-mask object frame was not frozen")
            observation = prepare_pose_mask_runtime_object_observation(
                images,
                masks,
                K,
                T_W2C,
                precomputed_object_frame=all_view_object_frame,
    ...
    input_quality = (
        ...
        in {
            "fixed_frame_names_valid_mask_after_all_view_o_v1",
            "time_axis_uniform_valid_mask_after_all_view_o_v1",
            "uniform_random_valid_mask_after_all_view_o_v1",
            "object_azimuth_balanced_valid_mask",
    ...
    report = {
        ...
        "input_frontend_format": (
            str(observation.condition_record["format"])
            if geometry_mode == "pose_mask"
        ...
        "all_input_view_count": int(len(frame_names_all)),
        "o_frozen_before_view_selection": bool(
            geometry_mode == "pose_mask"
            and observation.frame.stats.get("all_view_o_frozen_before_selection")
        ),
        "pose_mask_object_frame_policy": (
            str(frame_config.object_frame_policy)
            if isinstance(frame_config, PoseMaskObjectFrameConfig)
            else None
        ),
        "selected_source_view_indices": selected_indices.tolist(),
        "selected_frame_names": frame_names,
        "view_selection": selection_record,
        "reference_view_index": int(selected_reference_view_index),
        "o_reference_input_view_index": int(
            observation.frame.stats["axes"]["reference_view_index"]
        ),
        "o_reference_input_frame_name": str(
            observation.frame.stats.get("all_view_reference_frame_name", "")
        ),
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        default=LEGACY_Y_UP_OBJECT_FRAME_POLICY,
        help=(
            "versioned pose-mask model-O axis policy; the unified manual pipeline "
            "explicitly requests official_compatible_z_up_v1"
        ),
    )
    parser.add_argument("--selection_seed", type=int, default=20260819)
    parser.add_argument(
        "--fixed_frame_name",
        action="append",
        default=[],
        help=(
            "exact source frame name to replay; repeat exactly selected_view_count "
            "times and use with fixed_frame_names_valid_mask"
        ),
    )
    parser.add_argument(
        "--gravity_up_w",
...
def main() -> None:
    ...
    if args.geometry_mode == "pose_mask":
        frame_config = PoseMaskObjectFrameConfig(
            ...
            object_frame_policy=str(args.pose_mask_object_frame_policy),
        )
    else:
    ...
    view_selection_contract = {
        "fixed_frame_names_valid_mask": (
            "all-view-O-then-exact-named-frames-with-valid-mask-check"
        ),
        ...
        "time_uniform_valid_mask": (
            "all-view-O-then-time-uniform-with-valid-mask-fallback"
        ),
        "random_valid_mask": (
            "all-view-O-then-seeded-random-without-replacement-valid-masks"
        ),
        "training_spherical_farthest_valid_mask": (
    ...
    build_config = {
        ...
        "o_estimation_order": (
            "all eligible input views before model-view selection"
            if args.geometry_mode == "pose_mask"
            else "legacy point-mask path"
        ),
        "reference_view": (
            "largest_undistorted_foreground_mask_then_earliest_all-input-view"
            if args.geometry_mode == "pose_mask"
            else "legacy selected-view reference"
        ),
        "selection_seed": int(args.selection_seed),
        "fixed_frame_names": (
            list(args.fixed_frame_name)
    ...
    for index, row in enumerate(rows, start=1):
        ...
        try:
            report, was_reused = build_object_runtime_input(
                ...
                selection_seed=int(args.selection_seed),
                fixed_frame_names=list(args.fixed_frame_name),
            )
    ...
    if args.record_quality_rejections:
        ...
        eligibility_policy = {
            "runtime_frame_min_object_points": int(
                getattr(frame_config, "min_object_points", 0)
            ),
