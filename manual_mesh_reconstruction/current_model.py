...
from manual_mesh_reconstruction.model_inputs import (  # noqa: E402
...
from manual_mesh_reconstruction.mesh_coordinates import (  # noqa: E402
    ...
    *,
    ...
    for position, row in enumerate(rows):
        ...
        for seed in seeds:
            ...
            master_position = int(master_position_offset) + int(position)
            master_seed = int(seed) * 2000003 + master_position * 2017 + 7919
...
def make_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        default=0,
        help=(
            "Add this offset to the selected-row position used by the frozen "
            "coordinate-keyed SLat master-noise seed. The default preserves "
            "all existing behavior; nonzero values are only for an exact "
            "suffix repair after a recorded object-level decoder failure."
        ),
    )
    return parser
...
def main() -> None:
    ...
    if int(args.master_position_offset) < 0:
        raise ValueError("master_position_offset must be nonnegative")
    _configure_no_vggt_base()
    ...
    reports, slat_binding = _run_trained_slat(
        ...
        amp_dtype=amp_dtype,
        amp_enabled=amp_enabled,
        master_position_offset=int(args.master_position_offset),
    )
    manifest = {
    ...
    if int(args.master_position_offset) != 0:
        manifest["repair_master_position_offset"] = int(args.master_position_offset)
        manifest["repair_noise_contract"] = (
            "exact original worker-local master-noise positions"
        )
