...
# Two-GPU execution topology for the frozen Dev[16:64] VSS evaluation.
# This changes only the object sharding; every per-object seed/model contract
# is identical to the registered four-/six-GPU routes.
...
if (( ${#GPU_ARRAY[@]} != 2 )); then
  echo "ERROR: Dev48 two-GPU route requires exactly two GPUs" >&2
  exit 90
fi
STARTS=(16 40)
ENDS=(40 64)
