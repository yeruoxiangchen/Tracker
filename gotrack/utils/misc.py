...
import torch
import time
from gotrack.utils import structs, transform3d, logging
...
def save_per_frame_prediction(
    ...
    im_ids: Optional[np.ndarray] = None,
    ...
    # Handle missing scene_ids or im_ids
    num_objects = len(scores)
    if scene_ids is None:
        scene_ids = np.zeros(num_objects, dtype=int)
    if im_ids is None:
        im_ids = np.zeros(num_objects, dtype=int)
