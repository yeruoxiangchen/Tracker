...
from gotrack.utils import logging, structs, misc, transform3d, renderer_base
...
def write_text_on_image(
    ...
    loc = tuple(misc.clip_2d_point(torch.as_tensor(loc), torch.as_tensor(im_size)))
    ...
    for info in txt_list:
        ...
        if "val" in info:
            ...
            if "fmt" in info:
                val_tpl = "{" + info["fmt"] + "}"
            elif type(info["val"]) == float:  # type: ignore[comparison-overlap]  # noqa: E721
                val_tpl = "{:.3f}"
            else:
                val_tpl = "{}"
            if txt != "":
...
def vis_posed_meshes_of_objects(
    ...
    object_meshes_in_w = []
    for obj_idx, obj_lid in enumerate(object_lids):
        object_mesh = renderer.get_object_model(obj_lid).copy()

        obj_vertices = object_mesh.vertices * 1000

        object_mesh.vertices = transform3d.transform_points(
            misc.get_rigid_matrix(object_poses_m2w[obj_idx]), obj_vertices
        )
...
def create_object_mask(
    ...
    vertices = transform3d.transform_points(
        misc.get_rigid_matrix(object_poses_m2w[obj_idx]), obj_vertices
    )
...
def vis_scene_observation(
    scene_observation: structs.SceneObservation,
    renderer: renderer_base.RendererBase,
    colors: Dict[int, structs.Color],
) -> npt.NDArray:
    """Visualize the scene observation.

    Args:
        scene_observation: A SceneObservation object.
    Returns:
        A grid image of the scene observation.
    """
    ...
    vis_im = vis_posed_meshes_of_objects(
