...
from core.preprocess_BA import(
...
from core.global_optimize_deformationcontour_xiaoliehen_new_init_BA import optimize_global_pose, refine_model_with_deformation_graph

from core.refine_poes_guangliu import refine_global_pose_ba
...
def main() -> None:
    ...
    T_M2W_init_pure, s_fixed, _ = optimize_global_pose(
        all_optimization_data=[d for d in all_optimization_data if d["is_keyframe"]],
        all_corresps_data=[c for c in all_corresps_data if c is not None],
    ...
    logger.info(f"Final cost: {s_fixed}")
    ...
    #         K=data["K"],
    #         T_W2C=data["T_w2c"],
    #         T_M2W=T_M2W_init_pure,
    #         model_vertices=model_vertices,
    #         model_faces=model_faces,
    #     )

    # Stage 2: BA 精修
    T_M2W_final = refine_global_pose_ba(
        all_optimization_data=all_optimization_data,
        s_fixed=s_fixed,
        T_M2W_init=T_M2W_init_pure,
        model_vertices=model_vertices,
        model_faces=model_faces
    )
    for data in tqdm(all_optimization_data, desc="Final visualization"):
    ...
    refined_vertices = refine_model_with_deformation_graph(
        ...
        model_faces=model_faces,
        T_M2W=np.eye(4),
        all_optimization_data=all_optimization_data[::3],
        output_dir=output_dir,
    )
