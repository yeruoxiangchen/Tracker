...
class DinoFeatureExtractor(nn.Module):
    ...
    def __init__(self, model_name: str) -> None:
        ...
        # )
        # --- 替换 T.Normalize ---
        # 使用 register_buffer 存储 mean/std，以便它们能随模型一起转移到 GPU
        self.register_buffer(
            "norm_mean", 
            torch.tensor([0.485, 0.456, 0.406]).view(1, 3, 1, 1).float()
        )
        self.register_buffer(
            "norm_std", 
            torch.tensor([0.229, 0.224, 0.225]).view(1, 3, 1, 1).float()
        )
    ...
    def forward(self, images: torch.Tensor) -> tp.Dict[str, torch.Tensor]:
        ...
        images = (images - mean) / std # <--- 直接进行张量运算
