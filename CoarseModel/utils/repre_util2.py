...
def save_object_repre(
    ...
    for camera in repre.template_cameras_cam_from_model:
        cam_data = {
            ...
            "extrinsics": camera.extrinsics.cpu(),
        }
    ...
    if load_fields is None or "template_cameras_cam_from_model" in load_fields:
        for camera in object_dict["template_cameras_cam_from_model"]:
            camera_model = PinholePlaneCameraModel(
                ...
                width=camera["width"],
                height=camera["height"],
                # T_world_from_eye 在保存时被转换为张量，这里需要 to(tensor_device)
                T_world_from_eye=camera["T_world_from_eye"], 
            )
            if "extrinsics" in camera:
                camera_model.extrinsics = camera["extrinsics"].to(tensor_device)
                
            repre_dict["template_cameras_cam_from_model"].append(camera_model)
