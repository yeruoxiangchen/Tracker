...
def warp_box(
    ...
    corners = np.array([
        [box.left, box.top],     # top-left
        [box.right, box.top],    # top-right
        [box.right, box.bottom], # bottom-right
        [box.left, box.bottom],  # bottom-left
    ])

    eye_pts_src = src_camera.window_to_eye(corners)
    world_pts = src_camera.eye_to_world(eye_pts_src)
    eye_pts_dst = dst_camera.world_to_eye(world_pts)

    win_pts_dst = dst_camera.eye_to_window(eye_pts_dst)
    x_coords = win_pts_dst[:, 0]
    y_coords = win_pts_dst[:, 1]
