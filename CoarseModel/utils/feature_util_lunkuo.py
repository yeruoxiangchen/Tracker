...
from utils import dinov2_utils2 as dinov2_utils
...
def sample_mask_contour_points(
    mask: torch.Tensor,
    max_points: int,
    interior_ratio: float = 0.3,
):
    """
    mask: (H, W), torch.bool or {0,1}
    return: (N, 2) pixel coords
    """
    mask_np = mask.cpu().numpy().astype(np.uint8)
    # --- 1. 轮廓 ---
    contours, _ = cv2.findContours(
        mask_np, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE
    )
    contour_pts = np.concatenate(contours, axis=0).squeeze(1)

    # --- 2. 内部点（distance transform） ---
    dist = cv2.distanceTransform(mask_np, cv2.DIST_L2, 5)
    ys, xs = np.where(dist > dist.max() * 0.5)
    interior_pts = np.stack([xs, ys], axis=1)

    # --- 3. 按比例采样 ---
    n_contour = int(max_points * (1 - interior_ratio))
    n_interior = max_points - n_contour

    if contour_pts.shape[0] > n_contour:
        contour_pts = contour_pts[
            np.random.choice(contour_pts.shape[0], n_contour, replace=False)
        ]

    if interior_pts.shape[0] > n_interior:
        interior_pts = interior_pts[
            np.random.choice(interior_pts.shape[0], n_interior, replace=False)
        ]

    pts = np.concatenate([contour_pts, interior_pts], axis=0)
...
def get_visual_features_registered_in_3d(
    ...
    query_points = sample_mask_contour_points(
        object_mask_eroded,
        max_points=200,
    ).to(device)
