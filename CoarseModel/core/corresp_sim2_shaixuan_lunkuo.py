...
def extract_correspondences(
    ...
    for object_lid in object_lids:
        ...
        for inst_j, instance in enumerate(instances):
            ...
            #     # 5. 筛选阈值 (例如 5.0 像素，可根据 opts 调整)
            #     error_threshold = 5.0 
            #     inlier_mask = (reproj_errors < error_threshold) & valid_z

            #     # 6. 应用掩码更新所有相关数据
            #     logger.info(f"Filtering by reproj error: {len(final_q_2d)} -> {np.sum(inlier_mask)} points (thresh: {error_threshold}px)")
