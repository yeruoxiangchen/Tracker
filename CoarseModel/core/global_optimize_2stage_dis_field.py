...
# --- 1. 预处理：生成引力场 (Distance Field) ---

def preprocess_distance_fields(all_optimization_data):
    """
    为每一帧预计算距离变换图
    """
    print("--- Preprocessing Distance Fields ---")
    for data in all_optimization_data:
        mask_path = data['image_path'].replace("/rgb/", "/masks/").replace(".jpg", ".png")
        if not os.path.exists(mask_path):
            data['dist_map'] = None
            continue
            
        mask = cv2.imread(mask_path, cv2.IMREAD_GRAYSCALE)
        # 提取边缘 (也可以直接用 mask，取决于你的 mask 质量)
        edges = cv2.Canny(mask, 100, 200)
        
        # 计算距离变换：像素到最近边缘(0像素)的距离
        # cv2.distanceTransform 计算的是到 0 的距离，所以我们要反转边缘图
        dist_map = cv2.distanceTransform(255 - edges, cv2.DIST_L2, 5)
        
        # 截断距离 (可选)：防止远处的噪声产生过大的错误拉力
        dist_map = np.clip(dist_map, 0, 100)
        
        data['dist_map'] = dist_map.astype(np.float32)

# --- 2. 双线性插值采样 (关键：保证残差连续可导) ---

def sample_dist_field_bilinear(dist_map, pts_2d):
    """
    dist_map: (H, W)
    pts_2d: (N, 2) -> [u, v]
    """
...
def find_silhouette_vertices(V_current, N_3d, T_total, K):
    """
    寻找当前位姿下，Mesh 的轮廓顶点（法线与视线垂直的点）
    """
    # 转换到相机空间
    ...
    R_cam = T_total[:3, :3]
    N_cam = (R_cam @ N_3d.T).T
    
    # 视线向量 (从顶点指向相机，在相机空间即为 -V_cam)
    view_dir = -V_cam
    view_dir /= (np.linalg.norm(view_dir, axis=1, keepdims=True) + 1e-8)
    
    # 计算法线与视线的点积
    # dot ~ 0 表示该点位于轮廓边缘
    dot = np.sum(N_cam * view_dir, axis=1)
    
    # 选取轮廓点：通常取绝对值较小的点
    # 阈值 0.2 可以根据 Mesh 密度调整
    silhouette_mask = (np.abs(dot) < 0.2) & (V_cam[:, 2] > 0.1)
    return np.where(silhouette_mask)[0]
...
def refine_model_with_deformation_graph(
    ...
    t_start = time.time()
    # 只需执行一次
    preprocess_distance_fields(all_optimization_data)
    ...
    _, inverse_mapping = np.unique(np.round(V0, 6), axis=0, return_inverse=True)
    node_indices = farthest_point_sampling(V0, num_nodes)
    node_pos = V0[node_indices]
    graph_edges = np.array(build_knn_graph(node_pos, k=4))
    
    unique_idx = np.unique(inverse_mapping, return_index=True)[1]
    u_nodes, u_weights = compute_vertex_node_weights(V0[unique_idx], node_pos, k=4)
    vertex_nodes = u_nodes[inverse_mapping]
    vertex_weights = u_weights[inverse_mapping]
    ...
    current_x = np.zeros(6 * num_nodes, dtype=np.float64)
    ...
    for outer_iter in range(outer_iterations):
        ...
        V_current = apply_deformation_graph(V0, node_pos, R_list, t_list, vertex_nodes, vertex_weights)
        N_3d = compute_vertex_normals(V_current, model_faces)
        
        # B. 寻找每一帧的轮廓对应关系
        # 注意：这里我们不再存储 edge_2d，而是存储当前哪些 v_id 是轮廓点
        active_data = []
        for data in all_optimization_data:
            if data['dist_map'] is None: continue
            
            T_total = data['T_w2c'] @ T_M2W
            # 实时寻找轮廓顶点
            v_ids = find_silhouette_vertices(V_current, N_3d, T_total, data['K'])
            
            if len(v_ids) > 0:
                # 采样，防止点数过多拖慢优化
                if len(v_ids) > 500:
                    v_ids = np.random.choice(v_ids, 500, replace=False)
        ...
        # C. 最小二乘求解
        # 这里 lambda 可以随迭代动态减小（由粗到精）
        current_lambda = lambda_reg * (1.0 if outer_iter > 0 else 0.1)

        result = least_squares(
            deformation_graph_residual_field, # 使用新的残差函数
            current_x,
            method="trf",
            x_scale="jac",
            max_nfev=30,
            args=(V0, node_pos, vertex_nodes, vertex_weights, graph_edges, active_data, current_lambda)
        )
        current_x = result.x
        print(f"Iter {outer_iter+1} Cost: {result.cost:.4f}")
...
def prepare_batched_data_for_solver(all_optimization_data, T_M2W):
    ...
    for data in all_optimization_data:
        ...
        N_pts = len(v_ids)
        all_v_ids.append(v_ids)
        all_targets.append(data['edge_2d'])
        all_grads.append(data['edge_grad'])
        
        # 将位姿和内参复制扩展到和点数一致，方便后续做 Batch 矩阵乘法
        T_total = data["T_w2c"] @ T_M2W
        all_T_total.append(np.tile(T_total, (N_pts, 1, 1))) 
        all_K.append(np.tile(data["K"], (N_pts, 1, 1)))
        
    if not all_v_ids:
        return None
        
    return {
        'v_ids': np.concatenate(all_v_ids, axis=0),
        'T_total': np.concatenate(all_T_total, axis=0),
        'K': np.concatenate(all_K, axis=0),
        'targets': np.concatenate(all_targets, axis=0),
        'grads': np.concatenate(all_grads, axis=0)
    }
    
def deformation_graph_residual_field(x, V0, node_pos, v_nodes, v_weights, graph_edges, active_data, lambda_reg):
    ...
    for data in active_data:
        ...
        V_sub = V0[v_ids]
        # 计算形变
        V_def = apply_deformation_vectorized(V_sub, node_pos, R_list, t_list, v_nodes[v_ids], v_weights[v_ids])
        
        # 投影到 2D
        V_def_h = np.hstack([V_def, np.ones((len(V_def), 1))])
        V_c = (data['T_total'] @ V_def_h.T).T[:, :3]
        z = np.maximum(V_c[:, 2:3], 1e-6)
        proj = (data['K'] @ V_c.T).T[:, :2] / z
        
        # 从距离场采样残差
        dists = sample_dist_field_bilinear(data['dist_map'], proj)
        residuals.append(dists)

    # 2. ARAP 正则项 (保持原样)
    idx_k, idx_l = graph_edges[:, 0], graph_edges[:, 1]
