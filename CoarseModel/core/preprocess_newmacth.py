...
from core.corresp_newmatch import extract_correspondences
...
def build_optimization_data(
    ...
    for i, rgb_f in enumerate(rgb_files):
        ...
        if is_keyframe(i):
            # 1. 找候选模板
            candidates = find_candidate_templates(
                mask,
                precomputed_templates,
                template_ids,
                num_candidates=1,   # 直接 Top-1
            )
            ...
            if len(candidates) > 0:
                ...
                if corresp is not None:
                    ...
                    logger.info(
                        f"[Keyframe {i}] matched template {template_id}, "
                        f"#corresp={len(q_2d)}"
                    )
