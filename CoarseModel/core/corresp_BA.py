...
def extract_correspondences(
    ...
    for object_lid in object_lids:
        ...
        for inst_j, instance in enumerate(instances):
            ...
            # ================= 新增：feature map meta =================
            input_h, input_w = image_tensor_chw.shape[1], image_tensor_chw.shape[2]
            feat_h, feat_w = feature_map_chw.shape[1], feature_map_chw.shape[2]

            stride_w = input_w / feat_w
            stride_h = input_h / feat_h

            feat_map_meta = {
                "orig_camera": orig_camera_c2w,          # 原图 camera
                "crop_camera": camera_c2w,                # 当前使用的 camera（可能是 crop camera）
                "use_crop": opts.crop,

                # feature map 尺度信息
                "input_hw": (input_h, input_w),            # crop 图像大小
                "feat_hw": (feat_h, feat_w),               # feature map 大小
                "feat_stride": np.array(
                    [input_w / feat_w, input_h / feat_h],
                    dtype=np.float32
                ),
            }
            ...
            # 创建并返回此帧的对应数据
            return correspondence_data, feature_map_chw_proj, feat_map_meta