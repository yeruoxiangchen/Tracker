# core/preprocess.py
import os
import cv2
import numpy as np
import logging
import pickle

# from core.corresp import extract_correspondences
from core.corresp_sim2_final import extract_correspondences, extract_correspondences2
from core.util import (
    quat_to_rotmat, read_colmap_cameras_txt, 
    read_colmap_images_txt, colmap_camera_to_K
)
import torch
from utils import (
    feature_util, repre_util, knn_util
)

import trimesh

from .config import AppConfig

logger = logging.getLogger(__name__)

def save_optimization_cache(data, path):
    os.makedirs(os.path.dirname(path), exist_ok=True)
    with open(path, "wb") as f:
        pickle.dump(data, f)


def load_optimization_cache(path):
    with open(path, "rb") as f:
        data = pickle.load(f)
    return data["optimization_data"], data["corresps_data"]

def build_optimization_data(
    rgb_dir: str,
    mask_dir: str,
    colmap_dir: str,
    opts,
    start_idx: int = 0,       # 新增：开始索引
    end_idx: int = -1,      # 新增：结束索引（-1表示到最后）
    stride: int = 1,          # 新增：采样间隔（1表示每帧都取，2表示隔一帧取一次）
):
    """
    Returns:
        all_optimization_data: list[dict]
        all_corresps_data: list[CorrespondenceData]
    """
    cameras_colmap = read_colmap_cameras_txt(
        os.path.join(colmap_dir, "cameras.txt")
    )
    
    images_txt = os.path.join(colmap_dir, "images.txt")
    images_colmap = read_colmap_images_txt(images_txt)

    rgb_files = sorted(f for f in os.listdir(rgb_dir) if f.endswith((".jpg", ".png")))
    mask_files = sorted(f for f in os.listdir(mask_dir) if f.endswith(".png"))

    # 确定结束位置
    actual_end = len(rgb_files) if end_idx == -1 else min(end_idx, len(rgb_files))
    
    # 使用 Python 的切片语法 [start:stop:step] 快速筛选
    selected_rgb_files = rgb_files[start_idx:actual_end:stride]
    selected_mask_files = mask_files[start_idx:actual_end:stride]
    
    all_optimization_data = []
    all_corresps_data = []

    # ---------------- [优化：外部初始化] ----------------
    device = "cuda" if torch.cuda.is_available() else "cpu"
    # 1. 提取器只初始化一次
    logger.info("Pre-loading feature extractor...")
    extractor = feature_util.make_feature_extractor(opts.extractor_name)
    extractor.to(device)
    extractor.eval() # 确保是评估模式

    # 2. 加载对象表达并构建 KNN (假设 object_lids 只有一个，若有多个可做成 dict)
    object_lid = opts.object_lids[0] 
    repre_dir = os.path.join(AppConfig.OUTPUT_ROOT, "object_repre", 
                             opts.object_dataset, opts.version, str(object_lid))
    
    logger.info(f"Pre-loading representation for object {object_lid}...")
    repre = repre_util.load_object_repre(repre_dir=repre_dir, tensor_device=device)

    # 3. 预构建 KNN 索引
    logger.info("Pre-building KNN indices...")
    visual_words_knn_index = None
    if opts.match_template_type == "tfidf":
        visual_words_knn_index = knn_util.KNN(
            k=repre.template_desc_opts.tfidf_knn_k,
            metric=repre.template_desc_opts.tfidf_knn_metric
        )
        visual_words_knn_index.fit(repre.feat_cluster_centroids)

    template_knn_indices = []
    if opts.match_feat_matching_type == "cyclic_buddies":
        for template_id in range(len(repre.template_cameras_cam_from_model)):
            tpl_feat_mask = repre.feat_to_template_ids == template_id
            tpl_feat_ids = torch.nonzero(tpl_feat_mask).flatten()
            template_feats = repre.feat_vectors[tpl_feat_ids]
            template_knn_index = knn_util.KNN(k=1, metric="l2")
            template_knn_index.fit(template_feats.cpu())
            template_knn_indices.append(template_knn_index)
    # ---------------------------------------------------
    
    for rgb_file, mask_file in zip(selected_rgb_files, selected_mask_files):
        frame_name = rgb_file
        if frame_name not in images_colmap:
            logger.warning(f"{frame_name} not in COLMAP, skip")
            continue

        image_path = os.path.join(rgb_dir, rgb_file)
        mask_path = os.path.join(mask_dir, mask_file)

        image = cv2.cvtColor(cv2.imread(image_path), cv2.COLOR_BGR2RGB)
        mask = cv2.imread(mask_path, cv2.IMREAD_GRAYSCALE)
        if mask is None:
            mask = np.zeros(image.shape[:2], dtype=np.uint8)

        img_entry = images_colmap[frame_name]
        cam = cameras_colmap[img_entry["cam_id"]]
        K = colmap_camera_to_K(cam)

        corresp = extract_correspondences(
            image_raw=image,
            mask_raw=mask,
            camK=K,
            frame_name=frame_name,
            opts=opts,
            extractor=extractor,
            repre=repre,
            visual_words_knn_index=visual_words_knn_index,
            template_knn_indices=template_knn_indices,
            device=device
        )
        if corresp is None:
            continue

        # --- COLMAP T_w2c ---
        R_w2c = img_entry["R"]
        t_w2c = img_entry["t"]

        T_w2c = np.eye(4, dtype=np.float32)
        T_w2c[:3, :3] = R_w2c
        T_w2c[:3, 3] = t_w2c

        all_corresps_data.append(corresp)
        all_optimization_data.append(
            dict(
                frame_name=frame_name,
                K=K.astype(np.float32),
                T_w2c=T_w2c,
                q_2d=corresp.query_2d_pts.astype(np.float32),
                X_3d=corresp.model_3d_pts.astype(np.float32),
                X_ids=corresp.model_feat_ids,
                image_path=image_path,
            )
        )

    return all_optimization_data, all_corresps_data
# The `c` variable is being used as a placeholder for each candidate template ID along with
# its corresponding similarity score in the `find_candidate_templates` function. It is part of
# a list of tuples where each tuple contains the template ID and its similarity score with
# respect to the input image contour.

def build_optimization_data2(
    rgb_dir: str,
    mask_dir: str,
    colmap_dir: str,
    opts,
    mesh: trimesh.Trimesh,       # 新增：第一阶段优化好的 Mesh
    start_idx: int = 0,
    end_idx: int = -1,
    stride: int = 1,
):
    """
    Returns:
        all_optimization_data: list[dict]
        all_corresps_data: list[CorrespondenceData]
    """
    cameras_colmap = read_colmap_cameras_txt(os.path.join(colmap_dir, "cameras.txt"))
    images_txt = os.path.join(colmap_dir, "images.txt")
    images_colmap = read_colmap_images_txt(images_txt)

    rgb_files = sorted(f for f in os.listdir(rgb_dir) if f.endswith((".jpg", ".png")))
    mask_files = sorted(f for f in os.listdir(mask_dir) if f.endswith(".png"))

    actual_end = len(rgb_files) if end_idx == -1 else min(end_idx, len(rgb_files))
    selected_rgb_files = rgb_files[start_idx:actual_end:stride]
    selected_mask_files = mask_files[start_idx:actual_end:stride]
    
    all_optimization_data = []
    all_corresps_data = []

    # ---------------- [优化：外部初始化] ----------------
    device = "cuda" if torch.cuda.is_available() else "cpu"
    logger.info("Pre-loading feature extractor...")
    extractor = feature_util.make_feature_extractor(opts.extractor_name)
    extractor.to(device)
    extractor.eval()

    # 注意：这里删除了原来加载 repre 和预构建 KNN 的代码，因为我们要进行在线渲染匹配。
    # ---------------------------------------------------
    
    for rgb_file, mask_file in zip(selected_rgb_files, selected_mask_files):
        frame_name = rgb_file
        if frame_name not in images_colmap:
            logger.warning(f"{frame_name} not in COLMAP, skip")
            continue

        image_path = os.path.join(rgb_dir, rgb_file)
        mask_path = os.path.join(mask_dir, mask_file)

        image = cv2.cvtColor(cv2.imread(image_path), cv2.COLOR_BGR2RGB)
        mask = cv2.imread(mask_path, cv2.IMREAD_GRAYSCALE)
        if mask is None:
            mask = np.zeros(image.shape[:2], dtype=np.uint8)

        img_entry = images_colmap[frame_name]
        cam = cameras_colmap[img_entry["cam_id"]]
        K = colmap_camera_to_K(cam)

        # --- 将 T_w2c 提前计算，以便传递给 extract_correspondences ---
        R_w2c = img_entry["R"]
        t_w2c = img_entry["t"]
        T_w2c = np.eye(4, dtype=np.float32)
        T_w2c[:3, :3] = R_w2c
        T_w2c[:3, 3] = t_w2c

        # 模型到相机的最终位姿
        T_m2c = T_w2c 

        corresp = extract_correspondences2(
            image_raw=image,
            mask_raw=mask,
            camK=K,
            frame_name=frame_name,
            opts=opts,
            extractor=extractor,
            mesh=mesh,         # 传入 Mesh
            T_m2c=T_m2c,       # 传入实时位姿用于渲染
            device=device
        )
        if corresp is None:
            continue

        all_corresps_data.append(corresp)
        all_optimization_data.append(
            dict(
                frame_name=frame_name,
                K=K.astype(np.float32),
                T_w2c=T_w2c,
                q_2d=corresp.query_2d_pts.astype(np.float32),
                X_3d=corresp.model_3d_pts.astype(np.float32),
                X_ids=corresp.model_feat_ids,
                image_path=image_path,
            )
        )

    return all_optimization_data, all_corresps_data