...
def normalize_mesh_scale(
    ...
    target_max_extent: float = 200.0,
    min_allowed: float = 0.1,
    max_allowed: float = 1000.0,
    verbose: bool = True,
):
    """
    将 mesh 缩放到一个合理的物理尺度范围（单位：mm）

    参数:
        mesh: trimesh.Trimesh
        target_max_extent: 目标最大边长（mm），推荐 100~200
        min_allowed: 允许的最小尺寸（mm）
        max_allowed: 允许的最大尺寸（mm）
    """
    # 计算包围盒尺寸
    bbox_extents = mesh.bounding_box.extents
    max_extent = float(np.max(bbox_extents))
    if verbose:
        print(f"[Mesh] original max extent: {max_extent:.6f}")
    # 如果 mesh 尺寸异常，直接报错
    if max_extent < 1e-6:
        raise ValueError("Mesh size is too small or degenerate")
    # 如果已经在合理范围内，可以选择不缩放
    if min_allowed <= max_extent <= max_allowed:
        if verbose:
            print("[Mesh] scale is already in a reasonable range, no scaling needed")
        return mesh, 1.0
    # 计算缩放因子
    scale = target_max_extent / max_extent
    if verbose:
        print(
            f"[Mesh] scaling mesh by factor {scale:.6f} "
            f"(target max extent = {target_max_extent})"
        )
    mesh.apply_scale(scale)
    # 再次检查
    new_extent = np.max(mesh.bounding_box.extents)
    if verbose:
        print(f"[Mesh] new max extent: {new_extent:.6f}")
    return mesh, scale
...
class GenTemplatesOpts(NamedTuple):
    ...
    min_num_viewpoints: int = 162
    num_inplane_rotations: int = 1
...
def synthesize_templates(opts: GenTemplatesOpts) -> None: 
    ...
    # 构造 K 矩阵
    K_matrix = np.array([
        [FOCAL_LENGTH, 0.0, P_X],
        [0.0, FOCAL_LENGTH, P_Y],
        [0.0, 0.0, 1.0]
    ], dtype=np.float64)
    camera_input = {
    ...
    camera_input_width = camera_input['im_size'][0]
    camera_input_height = camera_input['im_size'][1]
    max_image_side = max(camera_input_width, camera_input_height)
    ...
    camera_model = PinholePlaneCameraModel(
        width=image_side,
        height=image_side,
        f=(camera_input['K'][0,0], camera_input['K'][1,1]),
        c=(
            camera_input['K'][0,2] - 0.5 * (camera_input_width - image_side),
            camera_input['K'][1,2] - 0.5 * (camera_input_height - image_side),
        )
    )
    ...
    # model_path = "/home/zjr/Tracker/foundpose/datasets/wogua/models/wogua.obj"
    # out_path = "/home/zjr/Tracker/foundpose/datasets/wogua/models/wogua_norm.obj"
    dataset_model_dir = os.path.join(AppConfig.DATASETS_PATH, opts.object_dataset, "models")
    model_path = os.path.join(dataset_model_dir, f"{opts.object_dataset}.obj")
    out_path = os.path.join(dataset_model_dir, f"{opts.object_dataset}_norm.obj")
    if not os.path.exists(dataset_model_dir):
        raise FileNotFoundError(f"Dataset model directory not found: {dataset_model_dir}")
    if not os.path.exists(model_path):
        raise FileNotFoundError(f"model not found: {model_path}")
    m = trimesh.load(model_path, force="mesh")
    m, scale_factor = normalize_mesh_scale(
        mesh=m,
        target_max_extent=200.0,   # 推荐 100~200
        min_allowed=0.1,
        max_allowed=1000.0,
        verbose=True,
    )
    m.export(out_path)
    print("Exported OBJ with texture:", out_path)
    # 根据模型边界框和相机参数计算 depth_range
    # 这里假设相机是针孔模型，fov_x = 2 * arctan(width / (2*f_x))
    camera_fov_y = 2 * np.arctan2(camera_input['im_size'][1]/2, camera_input['K'][1,1])
    camera_fov_x = 2 * np.arctan2(camera_input['im_size'][0]/2, camera_input['K'][0,0])

    # 模型尺寸
    model_bbox = m.bounding_box.extents
    model_diag = np.linalg.norm(model_bbox)

    # 设置 depth_range: 让整个模型在视野内，前后各加 10% padding
    depth_min = model_diag / (2 * np.tan(min(camera_fov_x, camera_fov_y)/2)) * 1.2
    depth_max = model_diag / (2 * np.tan(min(camera_fov_x, camera_fov_y)/2)) * 1.5
    ...
    # 模型
    model_tpath = out_path # "/home/zjr/Tracker/foundpose/datasets/wogua/models/wogua_mm.obj"
    ...
    for object_lid in object_lids:
        ...
        print("output_dir: ", AppConfig.OUTPUT_ROOT)
        ...
        for view_id, view in enumerate(views):
            ...
            for _ in range(opts.images_per_view):
                ...
                color = output[RenderType.COLOR]
                # 修正亮度范围
                color = np.clip(color, 0, 1)
                # gamma correction (linear → sRGB)
                color = np.power(color, 1/2.2)
                rgb_image = (color * 255).astype(np.uint8)
