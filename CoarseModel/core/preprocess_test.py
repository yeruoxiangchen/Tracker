...
def build_optimization_data(
    ...
    for rgb_file, mask_file in zip(rgb_files, mask_files):
        ...
        # --- 修改部分：前五帧逻辑控制 ---
        if is_keyframe(frame_count):
            corresp, feat_map = extract_correspondences(
                image_raw=image,
                mask_raw=mask,
                camK=K,
            ...
            # 如果前五帧提取失败，通常还是建议 skip 或者按 None 处理
            if corresp is None:
                logger.warning(f"Correspondence extraction failed for frame: {frame_name}")
                # 这里可以根据需求决定是 continue 还是填充 None
                # 如果要严格保持帧数对应，建议填充 None 而不是 continue
        else:
            corresp = None
        ...
        all_optimization_data.append(
            dict(
                ...
                feature_map_chw_proj=feat_map # 存入特征图供后续 BA 跟踪使用
            )
        )
