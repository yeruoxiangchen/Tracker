...
COARSE_CONNECT_DIR = os.path.join(COARSEMODEL_DIR, "connect")
COARSE_CORE_DIR = os.path.join(COARSEMODEL_DIR, "core")

# 独立 server 的运行态数据放在 trellis_point_prior_mv 下，不写入原
# ReconViaGen/ar_tracker，避免影响原 ReconViaGen + CoarseModel 流程。
BASE_DIR = os.path.join(TRACKER_ROOT, "trellis_point_prior_mv", "ar_tracker")
...
try:
    ...
    from CoarseModel.connect.sam2_mask import write_prompted_image_mask, write_prompted_video_masks

import ar_pose_quality as arq
from trellis_point_prior_mv.ar_session_smoke_io import append_slam_points_from_upload
...
def upload():
    ...
    try:
        ...
        try:
            ...
            print(f">>> [调度] [ARPoints] 点云保存失败但继续采集: {point_exc}", flush=True)
...
def _selected_indices_csv(indices):
    return ",".join(str(int(i)) for i in indices)


def _auto_prior_branch():
    requested = os.environ.get("TPP_PRIOR_BRANCH", "auto").strip()
    if requested and requested.lower() != "auto":
        return requested
    points_path = os.path.join(current_data_dir, "slam_points.jsonl")
    if os.path.exists(points_path) and os.path.getsize(points_path) > 0:
        return "ar_direct"
    return "pose_streaming"
...
def _finalize_capture_only(selected_indices, filtered_indices, input_qc_report):
    points_path = os.path.join(current_data_dir, "slam_points.jsonl")
    image_files = _list_session_images()
    mask_files = []
    if os.path.isdir(current_mask_dir):
        mask_files = sorted(
            name for name in os.listdir(current_mask_dir)
            if name.lower().endswith((".png", ".jpg", ".jpeg")) and "_prompt" not in name
        )
    report = {
        "status": "capture_only",
        "created_at": time.strftime("%Y-%m-%d %H:%M:%S"),
        "session_id": current_session_id,
        "session_data_dir": current_data_dir,
        "session_mask_dir": current_mask_dir,
        "session_preview_dir": current_preview_dir,
        "image_count": int(len(image_files)),
        "mask_count": int(len(mask_files)),
        "has_slam_points": bool(os.path.exists(points_path) and os.path.getsize(points_path) > 0),
        "slam_points_jsonl": points_path if os.path.exists(points_path) else "",
        "client_selected_indices": [int(i) for i in selected_indices],
        "filtered_selected_indices": [int(i) for i in filtered_indices],
        "input_qc": input_qc_report,
        "next_steps": {
            "pose_streaming_countcheck": (
                "SESSION_DATA_DIR='{data}' SESSION_MASK_DIR='{masks}' SELECTED_INDICES='{indices}' "
                "PRIOR_BRANCH=pose_streaming RUN_PREPARE=1 RUN_BUILD=1 RUN_EVAL=0 "
                "bash trellis_point_prior_mv/scripts/run_ar_session_smoke.sh"
            ).format(
                data=current_data_dir,
                masks=current_mask_dir,
                indices=_selected_indices_csv(filtered_indices),
            ),
            "ar_direct_countcheck": (
                "SESSION_DATA_DIR='{data}' SESSION_MASK_DIR='{masks}' SELECTED_INDICES='{indices}' "
                "PRIOR_BRANCH=ar_direct RUN_PREPARE=1 RUN_BUILD=1 RUN_EVAL=0 "
                "bash trellis_point_prior_mv/scripts/run_ar_session_smoke.sh"
            ).format(
                data=current_data_dir,
                masks=current_mask_dir,
    ...
    report_path = os.path.join(current_data_dir, "trellis_point_prior_capture_report.json")
    with open(report_path, "w") as f:
        json.dump(report, f, indent=2, ensure_ascii=False)
    report["capture_report"] = report_path
    return report


def _run_trellis_point_prior_generation(filtered_indices, input_qc_report):
    run_name = os.environ.get(
        "TPP_RUN_NAME",
        f"ar_session_{current_session_id}_pointprior_{time.strftime('%Y%m%d_%H%M%S')}",
    )
    out_root = os.environ.get(
        "TPP_OUT_ROOT",
        os.path.join(TRACKER_ROOT, "trellis_point_prior_mv", "outputs", "ar_session_server"),
    )
    run_dir = os.path.join(out_root, run_name)
    os.makedirs(run_dir, exist_ok=True)

    prior_branch = _auto_prior_branch()
    env = os.environ.copy()
    env.update(
        {
            "ROOT": TRACKER_ROOT,
            ...
            "RUN_PREPARE": "1",
            "RUN_BUILD": "1",
            "RUN_EVAL": os.environ.get("TPP_RUN_EVAL", "1"),
            "SELECTED_INDICES": _selected_indices_csv(filtered_indices),
            "GPU": os.environ.get("TPP_GPU", os.environ.get("GPU", "4")),
            "MAX_FRAMES": os.environ.get("TPP_MAX_FRAMES", "32"),
            "FRAME_SELECT": os.environ.get("TPP_FRAME_SELECT", "pose_random_farthest"),
            "MIN_PRIOR_POINTS": os.environ.get("TPP_MIN_PRIOR_POINTS", "20"),
            "TRI_MIN_OUTPUT_POINTS": os.environ.get("TPP_TRI_MIN_OUTPUT_POINTS", "20"),
            "TRI_MATCHER": os.environ.get("TPP_TRI_MATCHER", "sequential"),
            "TRI_MAX_PAIR_GAP": os.environ.get("TPP_TRI_MAX_PAIR_GAP", "4"),
            "TRI_ALLOW_PAIR_OUTSIDE_MASK": os.environ.get("TPP_TRI_ALLOW_PAIR_OUTSIDE_MASK", "1"),
            "TRI_MIN_PAIR_MATCHES": os.environ.get("TPP_TRI_MIN_PAIR_MATCHES", "8"),
            "MODES": os.environ.get("TPP_MODES", "stage2_correct"),
            "TOPK_SPECS": os.environ.get("TPP_TOPK_SPECS", "12000"),
            "MESH_EVAL_SAMPLES": os.environ.get("TPP_MESH_EVAL_SAMPLES", "2000"),
        }
    )
    if os.environ.get("TPP_STAGE2_CHECKPOINT"):
        env["STAGE2_CHECKPOINT"] = os.environ["TPP_STAGE2_CHECKPOINT"]
    if os.environ.get("TPP_PY"):
        env["PY"] = os.environ["TPP_PY"]
    ...
    with open(os.path.join(run_dir, "server_generation_request.json"), "w") as f:
        json.dump(request_report, f, indent=2, ensure_ascii=False)

    cmd = ["bash", os.path.join(TRACKER_ROOT, "trellis_point_prior_mv", "scripts", "run_ar_session_smoke.sh")]
    timeout = int(os.environ.get("TPP_GENERATE_TIMEOUT", "1800"))
    print(
        f">>> [TrellisPointPrior] 开始生成: run={run_name}, branch={prior_branch}, "
        f"indices={filtered_indices}",
        flush=True,
    )
    subprocess.run(cmd, cwd=TRACKER_ROOT, env=env, check=True, timeout=timeout)

    mesh_report = os.path.join(run_dir, "mesh_eval", "report.json")
    build_report = os.path.join(run_dir, "manifest", "build_report.json")
    prepare_report = os.path.join(run_dir, "prepare", "prepare_report.json")
    meshes = []
    if os.path.exists(mesh_report):
        try:
            with open(mesh_report, "r") as f:
                report = json.load(f)
            for row in report.get("rows", []):
                if row.get("mesh_obj"):
                    meshes.append(row["mesh_obj"])
        except Exception as exc:
            print(f">>> [TrellisPointPrior] mesh report 读取失败: {exc}", flush=True)
    return {
        "run_name": run_name,
        "run_dir": run_dir,
        "prior_branch": prior_branch,
...
def generate():
    ...
    print(f"\n>>> [TrellisPointPrior] 收到手机发来的保留序号 {selected_indices}", flush=True)
    image_files = _list_session_images()
    ...
    if not _env_flag("TPP_GENERATE_ENABLED", "0"):
        result = _finalize_capture_only(selected_indices, filtered_indices, input_qc_report)
        print(
            f">>> [TrellisPointPrior] TPP_GENERATE_ENABLED=0，仅保存采集数据: "
            f"{result['capture_report']}",
            flush=True,
        )
        return jsonify({
            "status": "success",
            "message": "Capture saved. Generation is disabled by default.",
            **result,
        }), 200

    try:
        print(">>> [TrellisPointPrior] TPP_GENERATE_ENABLED=1，开始当前模型生成...", flush=True)
        result = _run_trellis_point_prior_generation(filtered_indices, input_qc_report)
    except Exception as exc:
        import traceback
        traceback.print_exc()
        return jsonify({
            "status": "error",
            "message": f"Trellis point-prior generation failed: {exc}",
            "session_id": current_session_id,
            "session_data_dir": current_data_dir,
            "client_selected_indices": selected_indices,
            "filtered_selected_indices": filtered_indices,
    ...
    return jsonify({
        "status": "success",
        "message": "Trellis point-prior mesh generation finished.",
        "session_id": current_session_id,
        "session_data_dir": current_data_dir,
        "session_mask_dir": current_mask_dir,
        "client_selected_indices": selected_indices,
...
if __name__ == '__main__':
    clean_environment()
    host = os.environ.get("AR_SESSION_SERVER_HOST", "0.0.0.0")
    port = int(os.environ.get("AR_SESSION_SERVER_PORT", os.environ.get("PORT", "5000")))
    print(">>> [TrellisPointPrior Server] 独立 AR session server 启动")
    print(f">>> [TrellisPointPrior Server] runtime ar_tracker={BASE_DIR}")
    print(">>> [TrellisPointPrior Server] 不启动 ReconViaGen/run_local.py")
    print(f">>> [TrellisPointPrior Server] 默认只采集保存；TPP_GENERATE_ENABLED={os.environ.get('TPP_GENERATE_ENABLED', '0')}")

    print(f"服务器启动，作为调度器监听 {host}:{port} ...")
