...
from pose_aligned_reconstruction.aggregate_proobjaverse_official_native_ss_eval import (
    validate_record_matrix,
)
from pose_aligned_reconstruction.materialize_proobjaverse_official_ss_targets import (
...
class ProObjaverseOfficialSSTests(unittest.TestCase):
    ...
    def test_science_failed_candidate_requires_explicit_opt_in(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            path = self._deployment_report(Path(directory))
            payload = json.loads(path.read_text(encoding="utf-8"))
            payload["passed"] = False
            payload["checks"]["recall"] = False
            body = dict(payload)
            body.pop("report_sha256")
            payload["report_sha256"] = canonical_json_sha256(body)
            path.write_text(json.dumps(payload), encoding="utf-8")
            with self.assertRaisesRegex(ValueError, "identity/integrity"):
                load_official_native_ss_deployment(path)
            observed, binding = load_official_native_ss_deployment(
                path, require_science_passed=False
            )
            self.assertFalse(observed["passed"])
            self.assertEqual(binding["false_checks"], ["recall"])
