...
NINJA_WRAPPER_ROOT = TRACKER_ROOT / "pose_aligned_reconstruction" / "tools"
...
from pose_aligned_reconstruction.eval_direct_flow import (  # noqa: E402
    bootstrap_mean_ci,
    component_metrics,
    decode_coords,
    overlap_metrics,
...
from pose_aligned_reconstruction.prepare_direct_flow_mesh_protocol import (  # noqa: E402
    PROTOCOL_FORMAT,
    canonical_sha256,
    sha256_file,
)
from pose_aligned_reconstruction.train_direct_flow import build_direct_components  # noqa: E402
...
def load_canonical_gt(
    sample: dict[str, Any],
    *,
    canonical_margin_binding: dict[str, Any] | None = None,
    ...
    with np.load(latent_path) as payload:
        ...
        center_scale_fields = {"normalize_center", "normalize_scale"}
        bound_margin = None
        bound_provenance = None
        if canonical_margin_binding is not None:
            binding = dict(canonical_margin_binding)
            required_binding = {
            ...
            if mismatches:
                raise RuntimeError(f"invalid canonical-margin binding: {mismatches}")
            if Path(str(binding.get("ss_latent", ""))).resolve() != latent_path:
                raise RuntimeError("canonical-margin binding latent differs")
            if Path(str(binding.get("source_glb", ""))).resolve() != source_glb:
                raise RuntimeError("canonical-margin binding source GLB differs")
            for field in (
                ...
                if len(digest) != 64 or any(character not in "0123456789abcdef" for character in digest):
                    raise RuntimeError(f"canonical-margin binding has invalid {field}")
            bound_margin = float(binding.get("canonical_margin", float("nan")))
            if not math.isfinite(bound_margin) or bound_margin <= 0.0:
                raise RuntimeError("canonical-margin binding value is invalid")
            bound_provenance = binding
        if center_scale_fields.issubset(fields) and (
            "canonical_margin" in fields or bound_margin is not None
        ):
            center = np.asarray(payload["normalize_center"], dtype=np.float64)
            scale = float(payload["normalize_scale"])
            if center.shape != (3,) or not np.isfinite(center).all():
                raise RuntimeError("canonical normalize_center is invalid")
            if not math.isfinite(scale) or scale <= 0.0:
                raise RuntimeError("canonical normalize_scale is invalid")
            if "canonical_margin" in fields:
                margin = float(payload["canonical_margin"])
                if bound_margin is not None and not math.isclose(
                    margin, bound_margin, rel_tol=0.0, abs_tol=1.0e-7
                ):
                    raise RuntimeError("latent and bound canonical margins differ")
                margin_source = "ss_latent"
            else:
                margin = float(bound_margin)
                margin_source = "frozen_objaverse_render_manifest"
            if not math.isfinite(margin) or margin <= 0.0:
                raise RuntimeError("canonical_margin is invalid")
            frame_metadata = {
                "normalize_center": center.tolist(),
                "normalize_scale": scale,
                "canonical_margin": margin,
                "canonical_margin_source": margin_source,
                "frame": "canonical latent frame; no per-branch normalization or ICP",
            }
            if bound_provenance is not None:
                frame_metadata["canonical_margin_provenance"] = bound_provenance
        else:
            repair_format = str(payload["repair_format"]) if "repair_format" in fields else ""
            repair_target_mode = (
                str(payload["repair_target_mode"])
                if "repair_target_mode" in fields
                else ""
            )
            coordinate_frame = (
                str(payload["coordinate_frame"])
                if "coordinate_frame" in fields
                else ""
            )
            runtime_contract = {
            ...
            expected_runtime_contract = {
                ...
                "coordinate_frame": "runtime-O",
            }
            if runtime_contract != expected_runtime_contract:
                missing = sorted(canonical_fields - fields)
                raise ValueError(
                    "SS latent lacks canonical normalization and is not an audited "
                    f"runtime-O target: missing={missing}, contract={runtime_contract}"
                )
            recorded_source = sample.get("source_glb")
            if recorded_source and Path(str(recorded_source)).resolve() != source_glb:
                raise RuntimeError("runtime-O target mesh differs from cache manifest")
            expected_source_hash = str(sample.get("source_glb_sha256", ""))
            if expected_source_hash and sha256_file(source_glb) != expected_source_hash:
                raise RuntimeError("runtime-O target mesh hash differs from cache manifest")
            center = None
            scale = None
            margin = None
            frame_metadata = {
                **runtime_contract,
                ...
                "frame": (
                    "input-derived runtime-O Mesh label; no per-target normalization "
                    "or ICP"
                ),
            }
    ...
    if center is not None and scale is not None and margin is not None:
