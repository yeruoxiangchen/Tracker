...
def make_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--init_checkpoint",
        default="",
        help="initialize trainable weights for a new data identity; resets optimizer/step",
    )
...
def main() -> None:
    ...
    if args.resume:
        ...
        # A domain-adaptation run is a new optimization trajectory.  EMA starts
        # from exactly the selected parent weights; optimizer, scaler, step,
        # history, and RNG are intentionally not inherited.
        ema_state = initialize_ema_state(model)
