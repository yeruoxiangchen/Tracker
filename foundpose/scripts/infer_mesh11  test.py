...
def main() -> None:
    ...
    # images_txt = os.path.join(colmap_dir, 'images.txt')
    # images_colmap = read_colmap_images_txt(images_txt) # 假设返回字典
    #     # 尺度 s_init (使用 AABB 或其他方法计算)
    # X_W_obj = get_object_3d_points(
    #     colmap_points3D_path, 
    #     all_optimization_data, # 只传入当前帧数据
    #     mask_dir,
    #     images_colmap
    # )
    # s_init = calculate_initial_TMW_from_bbox(model_vertices, X_W_obj)
    s_init = 0.02829
    OUTPUT_FILE_PATH = "/home/zjr/Tracker/foundpose/results/refine_model/plane/res.txt"
    os.makedirs(os.path.dirname(OUTPUT_FILE_PATH), exist_ok=True)
    with open(OUTPUT_FILE_PATH, 'w') as f:
        # 文件头
        f.write("# Frame_Index | Initial_Cost | Final_Cost | Scale_Final | R_Init_flat | R_Final_flat | t_Init_flat | t_Final_flat\n")
        f.write("# R_flat is R[0,0], R[0,1], ..., R[2,2] (9 elements)\n")
        f.write("# t_flat is t[0], t[1], t[2] (3 elements)\n")
    for frame_idx in range(0, 48 + 1):
        print("-" * 50)
        print(f"[{frame_idx:04d}] Starting Optimization for Frame {frame_idx}")
        # T_m2c (从 PnP 结果或初始估计)
        R_m2c = all_corresps_data[frame_idx].best_pose["R_m2c"]
        t_m2c = all_corresps_data[frame_idx].best_pose["t_m2c"]
        T_m2c = np.eye(4)
        T_m2c[:3, :3] = R_m2c
        T_m2c[:3, 3] = t_m2c
        
        # T_w2c 和 T_c2w (从 COLMAP 结果)
        T_w2c = all_optimization_data[frame_idx]['T_w2c']
        ...
        # ## 测试是不是输入t全部大于0就好了
        # delta_t = np.maximum(0, -t_init) # 仅对负分量取绝对值
        # t_new_init = t_init + delta_t
        # R_init_inv = np.linalg.inv(R_init) 
        # model_offset = (R_init_inv @ delta_t) # 3x1 向量
        # # 3. 更新所有优化数据中的 3D 点 (关键步骤)
        # for data_entry in all_optimization_data:
        #     # 假设 'X_3d' 包含原始的 N x 3 模型点
        #     data_entry['X_3d_init'] = data_entry['X_3d']
        #     data_entry['X_3d'] = data_entry['X_3d'] - model_offset
        # t_init = t_new_init
        
        # 记录初始位姿的扁平化字符串
        R_init_flat = ' '.join(f'{x:.8f}' for x in R_init.flatten())
        t_init_flat = ' '.join(f'{x:.8f}' for x in t_init)
        ...
        cost_init = 0.5 * np.sum(res0**2)
        # lower_bounds = [1e-6, -np.inf, -np.inf, -np.inf, 0.0, 0.0, 0.0] 
        # upper_bounds = [np.inf, np.inf, np.inf, np.inf, np.inf, np.inf, np.inf]
        # 4. 调用非线性最小二乘优化
        # 注意：这里只将当前帧的数据传递给优化函数
        result = least_squares(
            fun=global_reprojection_error, 
            x0=initial_params, 
            args=(all_optimization_data,), # 只传递当前帧的数据
            method='trf', # Levenberg-Marquardt 算法
            # bounds=(lower_bounds, upper_bounds), # 设置边界，
            verbose=0, # 设置为0，避免在循环中打印过多信息
            # **(least_squares_options if least_squares_options else {})
        )
        ...
        # for data_entry in all_optimization_data:
        #     # 假设 'X_3d' 包含原始的 N x 3 模型点
        #     data_entry['X_3d'] = data_entry['X_3d_init']
            
        cost_final = result.cost
        
        # 扁平化最终位姿
        R_final_flat = ' '.join(f'{x:.8f}' for x in R_M2W_final.flatten())
        t_final_flat = ' '.join(f'{x:.8f}' for x in t_M2W_final)
        
        # 写入日志行
        log_line = (
            f"{frame_idx:04d}| {cost_init:.4e} | {cost_final:.4e} | "
            f"{s_final:.8f} | {R_init_flat} | {R_final_flat} | {t_init_flat} | {t_final_flat}\n"
        )
        ...
        T_M2W_final[:3, :3] = s_final * R_M2W_final  # S*R
        T_M2W_final[:3, 3] = s_final * t_M2W_final  # S*t

        # 使用第一帧的数据进行渲染
        first_frame_data = all_optimization_data[0]
        
        OUTPUT_VIS_DIR_ALLTEST = "/home/zjr/Tracker/foundpose/results/refine_model/plane/alltest"
        os.makedirs(OUTPUT_VIS_DIR_ALLTEST, exist_ok=True)
        visualize_projection(
            image_path=first_frame_data['image_path'],
            output_dir=OUTPUT_VIS_DIR_ALLTEST,
            output_name=f"{frame_idx:04d}.png", # 以当前优化帧索引命名
            K=first_frame_data['K'],
            T_W2C=first_frame_data['T_w2c'],
            T_M2W=T_M2W_final,
...
def visualize_projection(image_path: str, 
                         output_dir: str, 
                         K: np.ndarray, 
                         T_W2C: np.ndarray, 
                         T_M2W: np.ndarray, 
                         model_vertices: np.ndarray,
                         output_name: str = "projection_vis.png"):
    """
    将模型顶点通过给定的姿态链投影到图像上并可视化线框。

    Args:
        image_path (str): RGB 图像路径。
        output_dir (str): 输出图像保存目录。
        K (np.ndarray): 相机内参矩阵 (3x3)。
        T_W2C (np.ndarray): 世界坐标到相机坐标的变换矩阵 (4x4)。
        T_M2W (np.ndarray): 模型坐标到世界坐标的变换矩阵 (4x4)。
        model_vertices (np.ndarray): 模型顶点 (N, 3)。
        output_name (str): 保存的文件名。
    """
    ...
    # 2. 投影计算
    # 将 (N, 3) 顶点转换为 (4, N) 齐次坐标
    X_M_h = np.hstack([model_vertices, np.ones((model_vertices.shape[0], 1))]).T # 4 x N
    
    # 转换到相机坐标: X_C = T_M2C @ X_M
    X_C_h = T_M2C @ X_M_h # 4 x N
    X_C = X_C_h[:3, :] # 3 x N

    # 检查深度（仅保留前方点）
    valid_mask = X_C[2, :] > 1e-6
    X_C_valid = X_C[:, valid_mask]
    
    # 投影到像素坐标: uv_h = K @ X_C
    uv_h = K @ X_C_valid # 3 x N_valid
    
    # 归一化齐次坐标 (除以 Z/深度)
    Z = uv_h[2, :]
    uv_pixels = uv_h[:2, :] / Z # 2 x N_valid

    # 转换为 OpenCV 绘图格式 (N_valid, 2)
    uv_pixels = uv_pixels.T.astype(np.int32)
    
    # 3. 图像加载与绘制
    image = cv2.imread(image_path, cv2.IMREAD_COLOR)
    if image is None:
        logger.error(f"Could not load image from {image_path}. Skipping visualization.")
        return

    # 绘制模型顶点（作为点）
    for pt in uv_pixels:
        # 使用绿色圆点绘制
        cv2.circle(image, tuple(pt), radius=2, color=(0, 255, 0), thickness=-1)
        
    # 4. 保存结果
    os.makedirs(output_dir, exist_ok=True)
    output_path = os.path.join(output_dir, output_name)
    
    # OpenCV 默认使用 BGR 格式保存
