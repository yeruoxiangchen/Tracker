...
def global_reprojection_error(params, all_data_for_optimization):
    ...
    # M -> W 变换矩阵
    R_delta_c1w = axis_angle_to_rotmat(r_vec)
    T_delta_c1w = np.eye(4, dtype=np.float32)
    T_delta_c1w[:3, :3] = s * R_delta_c1w # 尺度只应用于旋转部分
    T_delta_c1w[:3, 3] = s * t_vec
    ...
    for data in all_data_for_optimization:
        ...
        X_3d_C1 = data['X_3d'] # 3D 模型点 (C1 坐标系)
        q_2d_obs = data['q_2d'] # 2D 观测点 (像素)
        
        # 1. 3D 点从 C1 坐标系变换到 W 坐标系
        # X_C1 (N, 3) -> X_W (N, 3)
        X_3d_C1_h = np.concatenate([X_3d_C1, np.ones((X_3d_C1.shape[0], 1))], axis=1) # 齐次坐标 (N, 4)
        X_3d_W_h = (T_delta_c1w @ X_3d_C1_h.T).T # (4, 4) @ (4, N) -> (4, N).T -> (N, 4)
...
def main() -> None:
    ...
    for data in all_optimization_data:
        ...
        for pt in pts_2d:
            x, y = pt
            if 0 <= x < w and 0 <= y < h:
                cv2.circle(vis_img, (x,y), radius=3, color=(255,0,0), thickness=-1)
    ...
    for data in all_optimization_data:
        X_m = data['X_3d']
        X_c = (R_m2c @ X_m.T+ t_m2c).T 
        data['X_3d'] = X_c  # 覆盖或可新建 data['X_3d_c']
    model_vertices_c = (R_m2c @ model_vertices.T+ t_m2c).T 
    
    # img1_p = all_optimization_data[0]['image_path']
    # img1=cv2.imread(img1_p,cv2.IMREAD_COLOR)
    # img1=cv2.cvtColor(img1,cv2.COLOR_BGR2RGB)
    # # cameras_txt_path = "/home/zjr/Tracker/foundpose/datasets/china/sparse/0/cameras.txt"
    # # K_dict = read_colmap_camera_K(cameras_txt_path)
    # # camK = K_dict[1]
    # KK=all_optimization_data[0]['K']
    # R = all_optimization_data[0]['T_w2c'][:3, :3]
    # t = all_optimization_data[0]['T_w2c'][:3, 3].reshape(3,1)
    # # Xxc = (R @ model_vertices.T + t).T  # (N,3)
    # Xxc = all_optimization_data[0]['X_3d']
    # uu = (KK[0,0] * Xxc[:,0] / Xxc[:,2]) + KK[0,2]
    # vv = (KK[1,1] * Xxc[:,1] / Xxc[:,2]) + KK[1,2]
    # pts_2d1 = np.stack([uu,vv], axis=1).astype(int)
    # vis_img1= img1.copy()
    # hh, ww = vis_img1.shape[:2]
    # for pt1 in pts_2d1:
    #     x, y = pt1
    #     if 0 <= x < ww and 0 <= y < hh:
    #         cv2.circle(vis_img1, (x,y), radius=3, color=(255,0,0), thickness=-1)
    # # 保存可视化结果
    # out_path = "/home/zjr/Tracker/foundpose/results/refine_model/china/model_trans/test.png"
    # cv2.imwrite(out_path, cv2.cvtColor(vis_img1, cv2.COLOR_RGB2BGR))
    
    # 确保 model_vertices 在优化前加载RGB
    from scipy.spatial.transform import Rotation
    T_w2c_first = np.eye(4)  # world -> camera
    # 随机小扰动：旋转 +/-5度, 平移 +/-0.001, 缩放 1 +/- 0.05
    # angle_perturb = np.deg2rad(0.5)
    # rot_perturb = Rotation.from_euler(
    #     'xyz', 
    #     np.random.uniform(-angle_perturb, angle_perturb, size=3)
    # ).as_matrix()
    # trans_perturb = np.random.uniform(-0.0001, 0.0001, size=3)
    # scale_perturb = 1.0 + np.random.uniform(-0.005, 0.005)
    # # 初始 Tw2m = COLMAP 第一帧 Tw2c * 扰动
    # Tw2m_init = np.eye(4, dtype=np.float32)
    # Tw2m_init[:3, :3] = (rot_perturb @ T_w2c_first[:3, :3])
    # Tw2m_init[:3, 3] = T_w2c_first[:3, 3] + trans_perturb
    
    # 固定小扰动参数
    angle_perturb_deg = np.array([1.0, -2.0, 0.5])  # xyz 旋转角度，单位为度
    angle_perturb = np.deg2rad(angle_perturb_deg)
    rot_perturb = Rotation.from_euler('xyz', angle_perturb).as_matrix()
    trans_perturb = np.array([0.005, -0.003, 0.002])  # 固定平移扰动
    scale_perturb = 1.0  # 固定缩放
    # 构造扰动后的 Tw2m
    Tw2m_init = np.eye(4, dtype=np.float32)
    Tw2m_init[:3, :3] = rot_perturb @ T_w2c_first[:3, :3]  # 旋转扰动乘在原旋转上
    Tw2m_init[:3, 3] = scale_perturb * (T_w2c_first[:3, 3] + trans_perturb)  # 平移+缩放
    # 4. 将模型顶点变换到模型空间
    model_vertices_m = (Tw2m_init[:3, :3] @ model_vertices_c.T).T + Tw2m_init[:3, 3]
    # model_vertices_m = model_vertices_c
    
    T_w2c = all_optimization_data[0]['T_w2c']
    ...
    T_delta_c1w = np.eye(4, dtype=np.float32)
    T_delta_c1w [:3, :3] = s_init * R_init 
    T_delta_c1w [:3, 3] = s_init * t_init
    
    T_gt = T_c2w @ np.linalg.inv(Tw2m_init)
    print("gt:\n",T_gt)
    print("init:\n",T_delta_c1w)
    ...
    R_delta_c1w_final = axis_angle_to_rotmat(final_params[1:4])
    t_delta_c1w_final = final_params[4:7]
    # s_final = s_init
    # R_delta_c1w_final = R_init
    # t_delta_c1w_final = t_init
    print(f"Optimized Scale (s): {s_final}")
    print(f"Optimized R_M2W:\n{R_delta_c1w_final}")
    print(f"Optimized t_M2W:\n{t_delta_c1w_final}")
    
    # # 4. 构造最终的W 矩阵
    T_delta_c1w_final = np.eye(4, dtype=np.float32)
    T_delta_c1w_final[:3, :3] = s_final * R_delta_c1w_final 
    T_delta_c1w_final[:3, 3] = s_final * t_delta_c1w_final
    
    # T_delta_c1w_final = T_delta_c1w
    
    logger.info("Starting visualization of optimized pose...")
    
    # --- 阶段 3: 最终可视化 (可选，使用最后一批次的 T_M2W) ---
    if T_delta_c1w_final is not None:
