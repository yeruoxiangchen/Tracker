...
def reprojection_error_Rt(rt_params, current_s, all_optimization_data):
    """
    固定 s，优化 R 和 t。
    rt_params: [rx, ry, rz, tx, ty, tz] (6 个参数)
    """
    s_params = np.array([current_s])
    full_params = np.concatenate([s_params, rt_params]) # 构造完整的 7 维参数
    # 调用原有的 7 维残差函数
    return global_reprojection_error(full_params, all_optimization_data)
def reprojection_error_s(s_params, current_r, current_t, all_optimization_data):
    """
    固定 R 和 t，优化 s。
    s_params: [s] (1 个参数)
    """
    full_params = np.concatenate([s_params, current_r, current_t]) # 构造完整的 7 维参数
    # 调用原有的 7 维残差函数
    return global_reprojection_error(full_params, all_optimization_data)
...
def main() -> None:
    ...
    data_cache_path = os.path.join(os.path.dirname(opts_path), "cached_optimization_data_infer10_plane.pkl")
    ...
    s_init = 1.0 # 可能得改
    # s_init = np.linalg.norm(t_m2c) / np.linalg.norm(T_w2c[:3,3])
    # t_init = T_m2w_init[:3,3] / s_init
    ...
    # --- 2. 替代 least_squares 的交替优化循环 ---
    def alternating_optimization(initial_params, all_optimization_data, max_iter=20, tolerance=1e-6):
        
        current_params = initial_params.copy()
        # 初始化 R, t 参数
        current_s = current_params[0]
        current_r = current_params[1:4]
        current_t = current_params[4:7]
        logger.info(f"Starting alternating optimization (Max Iter: {max_iter}, Tol: {tolerance})")
        for iteration in range(max_iter):
            # 记录上一次的 s 和 rt，用于检查收敛
            prev_s = current_s
            prev_r = current_r.copy()
            prev_t = current_t.copy()

            # === 步骤 1: 固定 s，优化 R 和 t ===
            logger.info(f"Iter {iteration+1} - Optimizing R, t (s={current_s:.6f} fixed)...")
            rt_init = np.concatenate([current_r, current_t])
            res_rt = least_squares(
                fun=reprojection_error_Rt, 
                x0=rt_init, 
                args=(current_s, all_optimization_data,), 
                method='lm',
                verbose=0
            )
            current_r = res_rt.x[0:3]
            current_t = res_rt.x[3:6]
            # === 步骤 2: 固定 R 和 t，优化 s ===
            logger.info(f"Iter {iteration+1} - Optimizing s (R, t fixed)...")
            s_init = np.array([current_s])
            # Bounds: s 必须为正
            res_s = least_squares(
                fun=reprojection_error_s, 
                x0=s_init, 
                args=(current_r, current_t, all_optimization_data,), 
                method='lm',
                # bounds=(0.0, np.inf), # 尺度必须大于0
                verbose=0
            )
            current_s = res_s.x[0]
            # === 检查收敛 ===
            delta_s = np.abs(current_s - prev_s)
            delta_r = np.linalg.norm(current_r - prev_r)
            delta_t = np.linalg.norm(current_t - prev_t)
            logger.info(f"Iter {iteration+1} - s change: {delta_s:.2e}, R change: {delta_r:.2e}, t change: {delta_t:.2e}")
            if delta_s < tolerance and delta_r < tolerance and delta_t < tolerance:
                logger.info("Alternating optimization converged.")
                break
        final_params = np.concatenate([[current_s], current_r, current_t])
        return final_params

    # 替换 main 函数中的 least_squares 调用
    final_params = alternating_optimization(initial_params, all_optimization_data, max_iter=20, tolerance=1e-7)
    # 3. 提取优化结果
