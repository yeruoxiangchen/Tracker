...
def synthesize_templates(opts: GenTemplatesOpts) -> None: 
    ...
    # 判断模型是否为单位米（假设模型尺寸 < 10 米认为是米单位）
    bbox_size = m.bounding_box.extents.max()
    if bbox_size < 10.0:
        print(f"模型单位疑似为米，自动转换为毫米，最大边长: {bbox_size} m")
        m.apply_scale(100.0)
    else:
        print(f"模型单位疑似为毫米，最大边长: {bbox_size} mm")
    # 导出转换后的模型
    out_path = "/home/zjr/Tracker/foundpose/datasets/wogua/models/wogua_mm.obj"
    # texture_path = "/home/zjr/Tracker/foundpose/datasets/wogua/models/wogua.jpg"
    # # m.export(out_path)
    # # 指定纹理图片
    # if hasattr(m.visual, 'material'):
    #     from PIL import Image
    #     m.visual.material.image = Image.open(texture_path)
    # 导出为 OBJ + MTL
    m.export(out_path)
    ...
    depth_min = model_diag / (2 * np.tan(min(camera_fov_x, camera_fov_y)/2)) * 0.9
    depth_max = model_diag / (2 * np.tan(min(camera_fov_x, camera_fov_y)/2)) * 1.1
