...
def main() -> None:
    ...
    for rgb_file, mask_file in tqdm(
        ...
        if corresp_data is not None:
            ...
            # pose_save_path = os.path.join(save_dir, "pose.txt")
    ...
    frame_name = first_data['frame_name']
    colmap_entry = images_colmap[frame_name]

    R_w2c = quat_to_rotmat(colmap_entry['qvec'])
    t_w2c = colmap_entry['tvec']
    T_w2c = np.eye(4)
    T_w2c[:3, :3] = R_w2c
    T_w2c[:3, 3] = t_w2c
    ...
    # points3d = read_colmap_points3d(points3d_txt)
    # if len(points3d) == 0:
    #     logger.error(f"points3D.txt is empty or not found at: {points3d_txt}")
    #     return
    # # 场景中心（COLMAP 世界坐标）
    # scene_center = np.mean(points3d, axis=0)
    # logger.info(f"Scene center (from COLMAP points3D): {scene_center}")
    # # === 使用第1帧的模型位姿与COLMAP数据估计初始T_M2W ===
    # R_m2c = all_corresps_data[0].best_pose["R_m2c"]  # 模型 -> 相机
    # t_m2c = all_corresps_data[0].best_pose["t_m2c"]
    # # 使用 COLMAP 世界中心作为初始平移
    # t_init = scene_center
    # # 初始旋转直接使用R_m2c
    # R_init = R_m2c.copy()
    # # 初始尺度 s_init = |t_m2c| / |scene_center|
    # norm_t_m2c = np.linalg.norm(t_m2c)
    # norm_t_center = np.linalg.norm(scene_center)
    # if norm_t_center < 1e-6:
    #     s_init = 1.0
    #     logger.warning("Scene center too close to origin, using s_init = 1.0")
    # else:
    #     s_init = norm_t_m2c / norm_t_center
    ...
    # # -------- 阶段1：仅优化尺度 s --------
    # def error_scale_only(params, all_data, R_init, t_init):
    #     s = params[0]
    #     full_params = np.array([s, 0, 0, 0, t_init[0], t_init[1], t_init[2]])
    #     return global_reprojection_error(full_params, all_data)

    # res_s = least_squares(
    #     fun=error_scale_only,
    #     x0=np.array([s_init]),
    #     args=(all_optimization_data, R_init, t_init),
    #     method='lm'
    # )
    # s_optimized = res_s.x[0]
    # logger.info(f"Step 1 done, s = {s_optimized}")
    # # -------- 阶段2：优化旋转和平移 --------
    # r_vec_init = Rotation.from_matrix(R_init).as_rotvec()
    # x0_refine = np.array([s_optimized, *r_vec_init, *t_init])
