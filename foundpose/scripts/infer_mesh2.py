...
def visualize_2d_2d_correspondences(
    ...
    n_tpls = min(len(all_q_crop), len(all_q_2d))
    if n_tpls == 0:
        print("[VIS] No valid q_crop/q_2d pairs for visualization.")
        return
    ...
    for i in range(n_tpls):
        ...
        # 文件名
        # crop_fname = os.path.join(output_dir, f"{base_frame}_tpl{i}_crop_pts.png")
        # orig_fname = os.path.join(output_dir, f"{base_frame}_tpl{i}_orig_pts.png")
        combined_fname = os.path.join(output_dir, f"{base_frame}_tpl{i}_combined.png")
...
def infer(opt_file_path, image_raw, mask_raw, camK, frame_name) -> None:
    ...
    for object_lid in object_lids:
        ...
        for inst_j, instance in enumerate(instances):
            ...
            # 提取 2D 观测点和对应的 3D 模型点信息
            all_q_crop = []
            all_q_2d = []
            all_X_3d = []
            all_X_ids = []
            all_scores = [] # 存储匹配分数 (coord_conf)
            
            if len(corresp) != 0:
                for tpl in corresp:
                    if tpl.get("coord_2d_ids", []).shape[0] == 0:
                        continue
                        
                    # 2D 观测点 (N, 2)
                    q_2d = tpl["coord_2d"].cpu().numpy()
                    # 3D 模型点坐标 (N, 3)
                    X_3d = tpl["coord_3d"].cpu().numpy()
                    # 3D 模型特征点 ID (N,)
                    X_ids = tpl["nn_vertex_ids"].cpu().numpy()
                    ...
                    if opts.crop:
                        ...
                        all_q_crop.append(q_2d)
                        q_2d = q_2d_in_orig  # 替换为映射到原图坐标的点

                    # 收集点
                    all_q_2d.append(q_2d)
                    all_X_3d.append(X_3d)
                    all_X_ids.append(X_ids)
                    all_scores.append(tpl["coord_conf"].cpu().numpy()) # 提取分数
                    # # 2D 观测点 (N, 2)
                    # all_q_2d.append(tpl["coord_2d"].cpu().numpy())
                    # # 3D 模型点坐标 (N, 3)
                    # all_X_3d.append(tpl["coord_3d"].cpu().numpy())
                    # # 3D 模型特征点的全局 ID (N,)
                    # all_X_ids.append(tpl["nn_vertex_ids"].cpu().numpy())
                
                ## 2d-2d验证
                visualize_2d_2d_correspondences(
                    all_q_crop=all_q_crop,
                    all_q_2d=all_q_2d,
                ...
                if len(all_q_2d) > 0:
                    final_q_2d = np.vstack(all_q_2d)
                    final_X_3d = np.vstack(all_X_3d)
                    final_X_ids = np.concatenate(all_X_ids)
                    all_scores = np.concatenate(all_scores)
                    # 获取唯一的 3D ID 及其首次出现的索引
                    # 'return_index=True' 给出每个唯一 ID 首次出现的索引
                    # 'return_inverse=True' 给出原始数组中元素到唯一 ID 数组的映射
                    ...
                    # 初始化一个数组来存储每个唯一 ID 对应的最佳匹配的索引
                    # 初始时，最佳匹配索引为首次出现的索引 (first_occurrence_indices)
                    best_match_indices = first_occurrence_indices.copy()
                    
                    # ----------------------------------------------------
                    # 步骤 3: 找出每个唯一 ID 中分数最高的匹配
                    # ----------------------------------------------------
                    ...
                    # # 输出目录
                    # save_dir = "/home/zjr/Tracker/foundpose/results/refine_model"
                    # os.makedirs(save_dir, exist_ok=True)

                    # # 输出文件名，可以用 frame_name 区分不同帧
                    # save_path = os.path.join(save_dir, f"{frame_name}_correspondences.png")

                    # # 绘制原图 + 2D 点可视化
                    # plt.figure(figsize=(10, 8))
                    # plt.imshow(image_raw)
                    # plt.scatter(final_q_2d[:, 0], final_q_2d[:, 1], s=2, c='r')
                    # plt.title(f"2D-3D correspondences on {frame_name}")
                    # plt.axis("off")
...
def insert_visualization_logic(
    frame_name, 
    inst_j, 
    q_2d_cropped, 
    q_2d_original, 
    image_np_hwc_cropped, 
    image_raw_original,
    image_path # 原始图像路径
):
    """
    插入到 infer 函数内部，用于验证 2D 点的投影和坐标还原。

    Args:
        frame_name: 当前帧的名称。
        inst_j: 当前实例的索引。
        q_2d_cropped: 裁剪图上的 2D 坐标 (N, 2)。
        q_2d_original: 还原到原图上的 2D 坐标 (N, 2)。
        image_np_hwc_cropped: 裁剪后的图像数据 (H, W, C)。
        image_raw_original: 原始图像数据 (H, W, C) 或从 image_path 加载。
        image_path: 原始图像文件的路径。
    """
    import matplotlib.pyplot as plt
    import matplotlib.image as mpimg
    VIS_OUTPUT_DIR = "/home/zjr/Tracker/foundpose/results/refine_model/wogua/2d-2d_test"
    os.makedirs(VIS_OUTPUT_DIR, exist_ok=True)
    # --- 可视化 1: q_2d 在裁剪图上的投影 (验证特征提取结果) ---
    plt.figure(figsize=(8, 6))
    plt.imshow(image_np_hwc_cropped) # 裁剪后的图像
    plt.scatter(
        q_2d_cropped[:, 0],
        q_2d_cropped[:, 1],
        s=5, 
        c='r', 
        label='2D Points (Cropped)'
    )
    plt.title(f"Cropped View: 2D Points ({frame_name}, Inst {inst_j})")
    plt.axis('off')
    plt.legend()
    save_path_cropped = os.path.join(VIS_OUTPUT_DIR, f"{frame_name}_inst{inst_j}_cropped_q2d.png")
    plt.savefig(save_path_cropped, bbox_inches='tight', dpi=200)
    plt.close()
    
# --- 可视化 2: q_2d_original 在原图上的投影 (验证坐标还原结果) ---

    # 尝试使用 mpimg.imread 加载图像，如果 image_raw 已经是 NumPy 数组，也可以直接使用
    if isinstance(image_raw_original, np.ndarray):
            orig_img = image_raw_original
    else:
            orig_img = mpimg.imread(image_path)
            
    plt.figure(figsize=(10, 8))
    plt.imshow(orig_img) # 原始图像
    plt.scatter(
        q_2d_original[:, 0], 
        q_2d_original[:, 1],
        s=5, 
        c='lime', # 使用鲜亮的颜色方便观察
        label='2D Points (Original)'
    )
    plt.title(f"Original View: Re-projected 2D Points ({frame_name}, Inst {inst_j})")
    plt.axis('off')
    plt.legend()
    save_path_orig = os.path.join(VIS_OUTPUT_DIR, f"{frame_name}_inst{inst_j}_original_q2d.png")
    plt.savefig(save_path_orig, bbox_inches='tight', dpi=200)
    plt.close()
...
def main() -> None:
    ...
    model_path = "/home/zjr/Tracker/foundpose/datasets/wogua/models/wogua_mm.ply" # 模型文件路径
    ...
    for rgb_file, mask_file in tqdm(
        ...
        if corresp_data is not None:
            ...
            # --- [ 新增可视化调用 ] ---
            visualize_projection2(current_data_entry, output_vis_dir)
            ...
            # plt.imshow(img)
            # plt.scatter(
            #     corresp_data.query_2d_pts[:, 0],
            #     corresp_data.query_2d_pts[:, 1],
            #     s=2, c='r'
            # )
            # plt.title(f"2D-3D correspondences: {frame_name}")
            # plt.axis('off')

            # # 输出文件路径
            # save_path = os.path.join(output_vis_dir, f"{frame_name}_2d_points.png")
            # plt.savefig(save_path, bbox_inches='tight', dpi=200)
            # plt.close()
    ...
    # 1. 初始化 T_M2W (假设 s=1, R=I, t=0 为初始猜测)
    s_init = 1.0
    R_init = np.eye(3)
    t_init = np.zeros(3)
