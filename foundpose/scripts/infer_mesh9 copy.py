...
def main() -> None:
    ...
    for i in range(len(all_optimization_data)):
        T_m2c = np.eye(4)
        T_m2c[:3, :3] = all_corresps_data[i].best_pose["R_m2c"]
        T_m2c[:3, 3] = all_corresps_data[i].best_pose["t_m2c"]

        T_w2c = all_optimization_data[i]['T_w2c']
        T_c2w = np.linalg.inv(T_w2c)
        T_m2w_hypo = T_c2w @ T_m2c

        all_t_m2w.append(T_m2w_hypo[:3, 3])
        all_R_m2w.append(T_m2w_hypo[:3, :3])
    t_hypotheses = np.array(all_t_m2w)
    # 沿 xyz 三个轴分别计算中位数
    t_init = np.median(t_hypotheses, axis=0) 
    print(f"Robust t_init: {t_init}")
    
    # distances = np.linalg.norm(t_hypotheses - t_init, axis=1)
    # best_init_idx = np.argmin(distances)

    # R_init = all_R_m2w[best_init_idx]
    # print(f"Using R_init from frame {best_init_idx}")
    print(f"Averaging {len(all_R_m2w)} rotations using robust quaternion method...")

    if not all_R_m2w:
        print("Warning: No rotation matrices found for averaging. Using identity.")
        R_init = np.eye(3)
    else:
        # 1. 将 3x3 旋转矩阵列表堆叠为 (N, 3, 3) 数组
        R_stack = np.stack(all_R_m2w, axis=0)
        
        # 2. 将所有旋转矩阵转换为四元数 (N, 4) 数组 [x, y, z, w]
        #    确保你已经 from scipy.spatial.transform import Rotation
        try:
            from scipy.spatial.transform import Rotation
            quats = Rotation.from_matrix(R_stack).as_quat()
        except ValueError as e:
            print(f"Error converting matrices to quaternions: {e}")
            print("This can happen if R_m2c from PnP is invalid (e.g., determinant != 1).")
            print("Falling back to identity matrix for R_init.")
            R_init = np.eye(3)
            quats = None

        if quats is not None:
            # 3. 处理 180 度模糊性：确保所有四元数都在同一个“半球”
            #    我们以第一个四元数为参考
            q_ref = quats[0]
            for i in range(1, quats.shape[0]):
                # 如果点积为负，意味着它们指向“相反”的方向
                if np.dot(q_ref, quats[i]) < 0:
                    quats[i] = -quats[i] # 翻转四元数 (q 和 -q 代表相同旋转)

            # 4. 构造 4x4 的协方差矩阵 M
            #    M = Q * Q^T, 其中 Q 是一个 4xN 矩阵，每列是一个四元数
            #    在 numpy 中, quats 是 (N, 4), 所以 M = quats.T @ quats
            M = quats.T @ quats

            # 5. 找到 M 的最大特征值对应的特征向量
            #    M 是对称半正定的，使用 np.linalg.eigh
            ...
            # 该特征向量就是平均后的四元数
            q_avg = eigvecs[:, max_eigval_idx]
            
            # 6. 将平均四元数转换回旋转矩阵
            R_init = Rotation.from_quat(q_avg).as_matrix()
            print(f"Robust R_init computed from quaternion averaging.")
