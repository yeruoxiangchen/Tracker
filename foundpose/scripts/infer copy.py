...
def infer(opts: InferOpts) -> None:
    ...
    bop_model_props = dataset_params.get_model_params(datasets_path=datasets_path, dataset_name=opts.object_dataset) # 加载这个物体数据集的 模型属性
    ...
    # Get properties of the test split of the specified dataset. # 为了后面加载测试集的 ground truth 和相机信息
    ...
    for object_lid in object_lids:
        ...
        # for scene_id in scene_ids:
            ...
            bop_im_id = item_info["im_id"] # 当前目标实例所在的图像 ID
            bop_chunk_id = item_info["scene_id"] # 当前目标实例所在的场景 ID
            ...
            # Get object annotations. 筛选图像中当前对象的标注，保证对象足够可见
            object_annos = []
            ...
            for inst_j, instance in enumerate(instances):
                ...
                # Iterate over the final poses to collect visuals. # 底下是评估位姿了
