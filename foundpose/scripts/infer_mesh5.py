...
# --- 核心误差函数 ---
def global_reprojection_error_fixed_s(params, s_fixed, all_data_for_optimization):
    """
    计算所有帧的总重投影误差（残差向量）。
...
def main() -> None:
    ...
    for batch_idx in range(num_batches):
        ...
        if T_M2W_current is None:
            # --- 第一批：初始化并确定固定的尺度 s_fixed ---
            first_data = current_batch_data[0]
            T_w2c_0 = first_data['T_w2c']
            T_c2w_0 = np.linalg.inv(T_w2c_0)
            
            # 使用 all_corresps_data[0].best_pose 进行初始化
            # 注意: all_corresps_data 的索引必须对应 first_data
            R_m2c = all_corresps_data[start_idx].best_pose["R_m2c"]
            t_m2c = all_corresps_data[start_idx].best_pose["t_m2c"]
            ...
            s_init = np.linalg.norm(R_init) / np.sqrt(3)  
            R_init /= s_init
            
            s_fixed = s_init 
            r_vec_init = Rotation.from_matrix(R_init).as_rotvec()
            
            # 优化参数只包含 [rx, ry, rz, tx, ty, tz] (6D)
            initial_params = np.array([r_vec_init[0], r_vec_init[1], r_vec_init[2], 
                                       t_init[0], t_init[1], t_init[2]], dtype=np.float64)
            
        else:
            # --- 后续批次：使用前一批次的 R 和 t 作为初始化，尺度 s 保持不变 ---
            s_prev = s_fixed # 使用固定的尺度
            R_prev = T_M2W_current[:3, :3] / s_prev
            t_prev = T_M2W_current[:3, 3]
            
            r_vec_prev = Rotation.from_matrix(R_prev).as_rotvec()
            
            initial_params = np.array([r_vec_prev[0], r_vec_prev[1], r_vec_prev[2], 
                                       t_prev[0], t_prev[1], t_prev[2]], dtype=np.float64)
            logger.info(f"Initialized with previous batch R/t (fixed s={s_fixed:.4f}).")

        # 2. 调用非线性最小二乘优化 (6D优化)
        result = least_squares(
            fun=global_reprojection_error_fixed_s,
            x0=initial_params, 
            args=(s_fixed, current_batch_data,),
            method='lm',
            verbose=2
        )
        ...
        s_final = s_fixed 
        R_M2W_final = axis_angle_to_rotmat(final_params[0:3])
        t_M2W_final = final_params[3:6]
        
        logger.info(f"Batch {batch_idx + 1} Optimized Scale (s): {s_final:.4f} (FIXED)")
        
        # 4. 构造最终的 T_M2W 矩阵并更新
        T_M2W_current = np.eye(4, dtype=np.float32)
        T_M2W_current[:3, :3] = s_final * R_M2W_final 
        T_M2W_current[:3, 3] = t_M2W_final
        T_M2W_final = T_M2W_current # 用于最终可视化
        
        print(f"Optimized Scale (s): {s_final}")
        print(f"Optimized R_M2W:\n{R_M2W_final}")
        print(f"Optimized t_M2W:\n{t_M2W_final}")
    
        # 5. 可视化 (每个 batch 结束后对该 batch 的帧进行可视化)
