...
def main() -> None:
    ...
    data_cache_path = os.path.join(os.path.dirname(opts_path), "cached_optimization_data_infer8_plane.pkl")
...
def visualize_projection_mesh(
    ...
    model_faces: np.ndarray,   # 渲染用 mesh faces
    color=(0, 255, 0)
):
    """
    将模型线框渲染到图像上（批量绘制，避免循环）。
    T_M2W 是 [sR | t]。
    """
    ...
    # 3. 投影
    q_2d_proj = project_points(K, X_3d_C).astype(np.int32)

    # 4. 深度过滤：保留在相机前方的三角形
    face_depth = X_3d_C[model_faces, 2]  # (F, 3)
    valid_faces = np.all(face_depth > 0, axis=1)
    faces_to_draw = model_faces[valid_faces]

    # 5. 批量生成多边形顶点列表
    pts = q_2d_proj[faces_to_draw]  # (F, 3, 2)
    # clip 到图像范围
    pts[..., 0] = np.clip(pts[..., 0], 0, w-1)
    pts[..., 1] = np.clip(pts[..., 1], 0, h-1)

    # reshape 为 polylines 可用格式
    pts_polylines = [p.reshape(-1,1,2) for p in pts]

    # 6. 批量绘制线框
    output_image = image_bgr.copy()
    cv2.polylines(output_image, pts_polylines, isClosed=True, color=color, thickness=1)
