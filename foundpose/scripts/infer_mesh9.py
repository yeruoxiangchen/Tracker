...
def infer(opt_file_path, image_raw, mask_raw, camK, frame_name) -> None:
    ...
    for object_lid in object_lids:
        ...
        for inst_j, instance in enumerate(instances):
            ...
            # +++ (新增：OmniGlue 查询特征提取) +++
            logger.info("Extracting query features using OmniGlue...")
            
            # 1. 提取 SP 和 DINO
            # (确保 image_np_hwc 是 np.uint8 HWC)
            ...
            sp_keypoints_query_np = sp_features_query[0] # (N, 2) [x, y]
            sp_desc_query = sp_features_query[1]
            
            # 2. 过滤掩码
            sp_keypoints_query_torch = torch.tensor(sp_keypoints_query_np, dtype=torch.float32, device=device)
            mask_modal_tensor = array_to_tensor(mask_modal).to(device)
            valid_indices_query = feature_util.filter_points_by_mask_indice(
                sp_keypoints_query_torch, mask_modal_tensor, return_indices=True
            )
            
            if valid_indices_query.shape[0] == 0:
                logger.info("No query SP keypoints in mask. Skipping.")
                continue

            query_kpts_torch = sp_keypoints_query_torch[valid_indices_query]
            query_sp_desc_torch = torch.tensor(sp_desc_query[valid_indices_query.cpu().numpy()], device=device)
            query_kpts_cpu = query_kpts_torch.cpu()
            query_kpts_np = query_kpts_cpu.numpy()
            query_kpts_tf = tf.convert_to_tensor(query_kpts_np, dtype=tf.float32)

            query_dino_desc = dino_extract.get_dino_descriptors(
                dino_features_query_dense,
                query_kpts_tf, # <--- 使用转换后的 TF 张量
                tf.convert_to_tensor(query_img_np_u8.shape[0], dtype=tf.int32), # height
                tf.convert_to_tensor(query_img_np_u8.shape[1], dtype=tf.int32), # width
                DINO_FEATURE_DIM,
            )
            query_dino_desc_torch = torch.tensor(query_dino_desc.numpy(), device=device)
            
            logger.info(f"Found {query_dino_desc_torch.shape[0]} valid query keypoints.")
        
            if (
                query_dino_desc_torch.shape[1] != repre.feat_vectors.shape[1]
                and len(repre.feat_raw_projectors) != 0
            ):
                logger.info(f"Applying PCA projection: {query_dino_desc_torch.shape[1]} -> {repre.feat_vectors.shape[1]}")
                
                # 使用 repre 中保存的投影器对查询特征进行投影
                query_dino_desc_torch = projector_util.project_features(
                    feat_vectors=query_dino_desc_torch,
                    projectors=repre.feat_raw_projectors,
                ).contiguous()
                
                logger.info(f"Query DINO features projected to PCA space. New shape: {query_dino_desc_torch.shape}")
            elif len(repre.feat_raw_projectors) != 0 and query_dino_desc_torch.shape[1] == repre.feat_vectors.shape[1]:
                logger.info("Query and template features already match dimensionality (likely both in PCA space).")
            else:
                # 既没有 PCA 投影器，维度也匹配，或没有 PCA 投影器但维度不匹配（这种情况不应该发生，但保留为默认情况）
                logger.info("No PCA projection applied to query features.")
                pass
            # +++ (新增：OmniGlue 粗匹配 - KNN "投票") +++
            logger.info("Performing Coarse Matching (KNN Vote)...")
            
            # 1. 搜索最近邻
            query_dino_desc_to_search = query_dino_desc_torch.cpu().contiguous()
            neighbor_indices, _ = knn_coarse_matching.search(query_dino_desc_to_search) # (N_query, k)
            # 2. 获取对应的模板 ID
            neighbor_indices_long = neighbor_indices.flatten().to(torch.long)
            neighbor_tpl_ids = repre.feat_to_template_ids[neighbor_indices_long] # (N_query * k,)
            # 3. 投票
            if neighbor_tpl_ids.shape[0] == 0:
                logger.info("Coarse matching found no neighbors. Skipping.")
                continue
            
            best_template_id = torch.mode(neighbor_tpl_ids).values.item()
            logger.info(f"Best template voted: {best_template_id}")
            
        #     # +++ (新增：计算 Top-N 候选模板以供可视化) +++
        # # 这一部分计算排名，为稍后的可视化做准备
        #     logger.info("Calculating Top-N templates for coarse match visualization...")
        #     TOP_N_VIS = 5 # (可调：您想看 Top-5 还是 Top-10？)
        #     # 1. 统计每个模板 ID 的得票数
        #     unique_tpl_ids, tpl_vote_counts = torch.unique(neighbor_tpl_ids, return_counts=True)
        #     # 2. 按票数降序排序
        #     sorted_counts, sorted_indices = torch.sort(tpl_vote_counts, descending=True)
        #     # 3. 获取 Top-N 的 ID 和票数
        #     top_n_indices = sorted_indices[:TOP_N_VIS]
        #     top_n_tpl_ids = unique_tpl_ids[top_n_indices].cpu().numpy()
        #     top_n_counts = sorted_counts[:TOP_N_VIS].cpu().numpy()
        #     # --- (这是您要求添加的代码) ---
        #     # +++ (新增：可视化 Top-N 粗匹配候选模板) +++
        #     # (确保 from omniglue.viz import visualize_matches 已经在文件顶部导入)
        #     # (确保 import matplotlib.pyplot as plt 已经在文件顶部导入)
        #     logger.info(f"Visualizing Top-{TOP_N_VIS} coarse match candidates...")
        #     import matplotlib.pyplot as plt # (如果已在顶部导入，此行可省略)
        #     for rank, (tpl_id, votes) in enumerate(zip(top_n_tpl_ids, top_n_counts)):
        #         # 1. 加载 Top-N 候选模板的图像
        #         candidate_tpl_image_chw = repre.templates[tpl_id]
        #         candidate_tpl_image_hwc_np = candidate_tpl_image_chw.permute(1, 2, 0).cpu().numpy()
        #         # 2. 调用可视化函数 (不显示关键点或匹配)
        #         # (我们复用 visualize_matches 来简单地拼接图像)
        #         viz_coarse = visualize_matches( 
        #             image0=query_img_np_u8,           # 查询图像
        #             image1=candidate_tpl_image_hwc_np, # 候选模板图像
        #             kp0=np.array([]),                 # 无
        #             kp1=np.array([]),                 # 无
        #             match_matrix=np.array([]),        # 无
        #             show_keypoints=False,
        #             highlight_unmatched=False,
        #             title=f"Coarse Match Rank {rank+1} (Tpl {tpl_id}) - Votes: {votes}",
        #             line_width=0,
        #             circle_radius=0,
        #             circle_thickness=0
        #         )
        #         # 3. 保存图像
        #         # (确保 infer 函数能接收到 frame_name 变量)
        #         is_best_str = "_BEST" if tpl_id == best_template_id else ""
        #         output_filename = f"/home/zjr/Tracker/foundpose/results/refine_model/plane/2d-2d-omni/coarse_viz_{frame_name}_rank{rank+1}_tpl{tpl_id}{is_best_str}.png"
        #         plt.figure(figsize=(20, 10), dpi=150)
        #         plt.axis("off")
        #         plt.imshow(viz_coarse)
        #         plt.savefig(output_filename, bbox_inches='tight', pad_inches=0)
        #         plt.close() # 关闭图像

        #     logger.info(f"...Top-{TOP_N_VIS} coarse match visualization saved.")
        # +++ (可视化结束) +++
        # --- (添加的代码结束) ---
            # +++ (新增：OmniGlue 精匹配) +++
            ...
            # 2. 反向投影
            model_3d_pts_torch = unproject_pts_to_3d(
                match_kp_template, # (N, 2) [x, y] numpy
                depth_image_hw,
                template_cam,
                T_model_from_camera,
                device
            ...
            # # --- 在 infer 函数中调用 ---
            # # 在去重后的 final_q_2d 和 final_X_3d 之后
            # output_dir_vis = "/home/zjr/Tracker/foundpose/results/refine_model/plane/2d_3d_valid"
            # visualize_2d_3d_correspondences(
            #     image_np_hwc=orig_image_np_hwc,
            #     q_2d=final_q_2d,
            #     X_3d=final_X_3d,
            #     R_m2c=best_pose["R_m2c"],
            #     t_m2c=best_pose["t_m2c"].reshape(3,1),
            #     camK=camK,
            #     output_path=output_dir_vis,
...
def main() -> None:
    ...
    model_path = os.path.join(base_dir, "models/plane.ply")
    output_vis_dir = "/home/zjr/Tracker/foundpose/results/refine_model/plane"
    data_cache_path = os.path.join(os.path.dirname(opts_path), "cached_optimization_data_infer9_plane.pkl")
    ...
    #         image_path=image_path,
    #         output_dir="/home/zjr/Tracker/foundpose/results/refine_model/plane/init_model",
    #         K=K,
    #         T_W2C=T_W2C,
    #         T_M2W=T_m2w_init,
