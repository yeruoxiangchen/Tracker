...
def main() -> None:
    ...
    # # 1. 初始化 T_M2W
    first_data = all_optimization_data[20]

    R_m2c = all_corresps_data[20].best_pose["R_m2c"]
    t_m2c = all_corresps_data[20].best_pose["t_m2c"]
    T_m2c = np.eye(4)
    T_m2c[:3, :3] = R_m2c
    T_m2c[:3, 3] = t_m2c
    ...
    T_M2W_final0 = np.eye(4, dtype=np.float32)
    T_M2W_final0[:3, :3] = s_init * R_init 
    T_M2W_final0[:3, 3] = s_init * t_init
    # visualize_projection(
    #             image_path=all_optimization_data[0]['image_path'],
    #             output_dir=output_vis_dir,
    #             K=all_optimization_data[0]['K'],
    #             T_W2C=T_w2c,
    #             T_M2W=T_M2W_final0,
    #             model_vertices=X_c
    #         )
