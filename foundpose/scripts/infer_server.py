...
def pre_infer(opt_file_path, object_lid):
    ...
    # Build a kNN index from object feature vectors.建立全局 KNN 索引 快速找到与图像特征最相似的模板
    ...
    # Build per-template KNN index with features from that template. 根据图像特征判断更可能匹配到哪个模板，再用对应的索引来查找候选点。
...
def infer(opts, extractor, foundpose_detection, camK, object_lid, repre, template_knn_indices, visual_words_knn_index, image_raw):
