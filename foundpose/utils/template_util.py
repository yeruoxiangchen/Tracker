...
def calc_tfidf(
    ...
    idf = word_idfs.to(tf.device)[feature_word_ids_flat]
...
def tfidf_matching(
    ...
    # 更改设备
    target_device = query_features.device
    word_ids = word_ids.to(target_device)
    word_dists = word_dists.to(target_device)
    ...
    query_tfidf = query_tfidf.to(object_repre.template_descs.device)    # device 问题
