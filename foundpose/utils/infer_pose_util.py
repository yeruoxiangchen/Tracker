...
def get_instances_for_pose_estimation2( 
    obj_id: int,
    use_detections: bool,
    detections: Dict[Any, Any],
    image_size: Tuple[int, int],
    ...
    if use_detections:
        ...
        for pred in preds:
            ...
            shift_x = 0
            if image_size[0] < orig_size[0]:
                shift_x = (orig_size[0] - image_size[0]) // 2
            elif image_size[0] > orig_size[0]:
                raise ValueError("Image is larger than mask.")
            shift_y = 0
            if image_size[1] < orig_size[1]:
                shift_y = (orig_size[1] - image_size[1]) // 2
            elif image_size[1] > orig_size[1]:
                raise ValueError("Image is larger than mask.")

            # Apply the shift to bbox
            box_amodal[0] -= shift_x
            box_amodal[1] -= shift_y
