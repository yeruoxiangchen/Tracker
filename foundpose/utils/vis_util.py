...
def vis_inference_results(
    ...
    if not vis_for_teaser:
        ...
        # bbox
        vis = np.array(base_image).copy()
        x1, y1, x2, y2 = map(int, object_box)  # assume object_box is [x1, y1, x2, y2]
        cv2.rectangle(vis, (x1, y1), (x2, y2), color=(0, 255, 255), thickness=2)
        cv2.putText(vis, "bbox", (x1, max(y1 - 5, 0)), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 255), 2)

        vis_base_util.plot_images(imgs=[vis], dpi=dpi)
        if not vis_for_paper:
            vis_base_util.add_text(0, "Input bbox")
        vis_tile_left = vis_base_util.save_plot_to_ndarray()
