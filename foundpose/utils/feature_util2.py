...
def filter_points_by_mask_indice(
    points: torch.Tensor, 
    mask: torch.Tensor,
    return_indices: bool = False  # <-- 新增参数，并设默认值
) -> Union[torch.Tensor, torch.Tensor]: # <-- 修改返回类型
    """Keeps only points inside the specified mask.

    Args:
        points: 2D coordinates of shape (num_points, 2).
        mask: A binary mask of shape (H, W).
        return_indices: If True, returns the indices of valid points instead 
                        of the points themselves.
    Returns:
        Filtered points (if return_indices=False) or 
        Valid indices (if return_indices=True).
    """
    ...
    # 1. 创建画布边界的掩码 (in-box mask)
    h, w = mask.shape
    x_coords = points_int[:, 0]
    y_coords = points_int[:, 1]
    
    in_box_mask = (x_coords >= 0) & (x_coords < w) & \
                  (y_coords >= 0) & (y_coords < h)
    
    # 初始化最终掩码，默认为全 False
    final_valid_mask = torch.zeros_like(in_box_mask)

    # 2. 获取在画布边界内的点
    points_int_in_box = points_int[in_box_mask]
    
    if points_int_in_box.shape[0] > 0:
        # 3. 检查这些点在物体掩码 (mask) 上的值
        # [y, x] 索引
        mask_values_at_points = mask[points_int_in_box[:, 1], points_int_in_box[:, 0]].bool()
        
        # 4. 更新最终掩码
        # masked_scatter_ 会在 in_box_mask 为 True 的位置
        # 写入 mask_values_at_points 的值
        final_valid_mask.masked_scatter_(in_box_mask, mask_values_at_points)

    # 5. 根据标志返回
    if return_indices:
        return torch.where(final_valid_mask)[0] # 返回索引
    else:
        return points[final_valid_mask] # 返回过滤后的点
