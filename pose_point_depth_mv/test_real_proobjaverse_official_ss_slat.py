...
class DeploymentDefaultTest(unittest.TestCase):
    ...
    def test_mobile_capture_and_server_record_unfiltered_pose_audit_contract(self) -> None:
        ...
        self.assertNotIn("EvaluatePoseDiverseCandidate", client)
        self.assertIn("enablePoseDiverseCapture = false", client)
        self.assertIn("minimumPoseDiversityAngleDegrees = 10.0f", client)
        self.assertIn('"capture_view_policy"', client)
        self.assertIn('"fixed_interval_unfiltered_0p2s_v1"', client)
        self.assertIn('"capture_frame_filtering_applied", "false"', client)
