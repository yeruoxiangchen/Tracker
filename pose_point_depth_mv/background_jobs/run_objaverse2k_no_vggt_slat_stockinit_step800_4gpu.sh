...
GPUS=${OBJAVERSE2K_STOCKINIT_GPUS:-1,2,6,7}
...
OUT=${RUN}/slat_objaverse2135_stockinit_step2000_seed42_4gpu_v1
STATE=${RUN}/logs/slat_objaverse2135_stockinit_step800_4gpu.state
EXIT_CODE=${RUN}/logs/slat_objaverse2135_stockinit_step800_4gpu.exit_code
LOCK=${RUN}/logs/slat_objaverse2135_stockinit_step800_4gpu.lock

IFS=',' read -r -a GPU_ARRAY <<<"${GPUS}"
if [ "${#GPU_ARRAY[@]}" -ne 4 ] || \
   [ "$(printf '%s\n' "${GPU_ARRAY[@]}" | sort -u | wc -l)" -ne 4 ]; then
  echo "OBJAVERSE2K_STOCKINIT_GPUS requires four distinct GPUs" >&2
  exit 96
fi
...
exec 9>"${LOCK}"
if ! flock -n 9; then
  echo "Objaverse2K Stock-init step800 training is already running" >&2
  exit 99
fi
finish() {
  RC=$?
...
trap finish EXIT
printf 'started_at=%s state=running gpus=%s run_until_step=%s initialization=stock\n' \
...
if [ -s "${OUT}/checkpoints/step_000800.pt" ] && \
  ...
  echo "Objaverse2K Stock-init step800 training already complete: ${OUT}"
else
  if [ -s "${OUT}/checkpoints/last.pt" ]; then
    START=(--resume "${OUT}/checkpoints/last.pt")
  elif [ -e "${OUT}" ]; then
    echo "Stock-init output exists without resumable last.pt: ${OUT}" >&2
    exit 98
  else
...
assert args["grad_accum"] == 2 and args["seed"] == 42
