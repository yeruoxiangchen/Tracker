...
GPUS_CSV=${EVAL_GPUS:-0,1,2,3,5,6,7}
SEED=${EVAL_SEED:-42}
...
WORKERS=${#GPU_ARRAY[@]}
if (( WORKERS != 7 )); then
  echo "ERROR: registered Dora300 run requires exactly seven GPUs; got ${GPUS_CSV}" >&2
  exit 90
fi
if [[ "$(printf '%s\n' "${GPU_ARRAY[@]}" | sort -u | wc -l)" -ne 7 ]]; then
  echo "ERROR: EVAL_GPUS contains duplicates: ${GPUS_CSV}" >&2
  exit 91
fi
...
echo "===== P0 exact frozen Dora model-O runtime (CPU; GT mesh not read) ====="
...
echo "===== P1 launch ${WORKERS} SS30K+SLat30K workers on GPUs ${GPUS_CSV} ====="
pids=()
...
PY
  ...
  if (( ${#object_keys[@]} < 42 || ${#object_keys[@]} > 43 )); then
    echo "ERROR: worker ${worker} Dora object count differs: ${#object_keys[@]}" >&2
    exit 92
  fi
  object_args=()
  ...
  (
    ...
    echo "[worker ${worker}] SS30K + SLat30K + frozen Stock Mesh decoder"
    CUDA_VISIBLE_DEVICES="${gpu}" \
...
for worker in "${!pids[@]}"; do
  if ! wait "${pids[$worker]}"; then
    echo "ERROR: Dora model worker ${worker} failed; completed outputs are resumable" >&2
    failed=1
  fi
done
(( failed == 0 )) || exit 93

echo "===== P2 aggregate Dora300 ====="
"${PY}" -u -m pose_point_depth_mv.evaluate_dorabench300_ss30k_slat30k \
  aggregate \
  --benchmark_manifest "${BENCHMARK}" \
...
echo "DORA300 SS30K+SLAT30K METRICS COMPLETE: ${OUT}/aggregate_v1/report.json"
