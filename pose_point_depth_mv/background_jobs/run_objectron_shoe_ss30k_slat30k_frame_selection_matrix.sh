...
OUTPUT=${OUTPUT:-${PROJECT_ROOT}/pose_point_depth_mv/outputs2/ObjectronShoe_obj0_三选帧_ReconViaGen_vs_SS30K_SLat30K_双RuntimeO轮廓_20260819_v1}
MATRIX_GPUS=${MATRIX_GPUS:-3,4,5,6,7}
OBJECT_KEY=objectron_shoe:shoe_batch14_30_obj0
...
if [[ -s "${PLAN}" ]]; then
  ...
  test ! -e "${OUTPUT}"
  echo "===== CPU PREPARE: Objectron shoe object0 / selections / paired runtime-O ====="
  "${PYTHON}" -u -m pose_point_depth_mv.objectron_ss30k_slat30k_frame_selection_matrix \
    prepare --dataset "${DATASET}" --output "${OUTPUT}" \
    --clip-sequence shoe/batch-14/30 --official-object-id 0 \
    --object-category objectron_shoe --object-id shoe_batch14_30_obj0 \
    --experiment-label "Objectron Shoe object_id=0" \
    --allow-phone-diagnostic
fi
mkdir -p "${OUTPUT}/logs"

build_pose_model_input() {
  local slug=$1 gpu=$2 root=${OUTPUT}/$1
  if [[ -s "${root}/03_model_input_pose_mask/model_input_manifest.json" ]]; then
    echo "[reuse DINO-only model-input] ${slug}"
    return 0
  fi
...
echo "===== GPU PHASE 1: three exact DINO-only encodes ====="
pids=()
...
phase_rc=0
for pid in "${pids[@]}"; do if ! wait "${pid}"; then phase_rc=1; fi; done
if ((phase_rc != 0)); then
  echo "ERROR: DINO-only model-input phase failed; inspect phase1 logs" >&2
  exit 91
fi

echo "===== CPU PAIRING: exact image contexts + official object-pose geometry ====="
...
run_recon() {
  local slug=$1 gpu=$2 root=${OUTPUT}/$1
  if [[ -s "${root}/07_reconviagen_once/inference_manifest.json" ]]; then
    echo "[reuse strict ReconViaGen] ${slug}"
    return 0
  fi
  CUDA_VISIBLE_DEVICES="${gpu}" "${PYTHON}" -u -m \
...
echo "===== GPU PHASE 2: all 3 selections x both O, plus ReconViaGen once ====="
(
  run_current 01_time_uniform8 pose_mask "${GPUS[0]}"
  run_recon 03_random8_seed20260819 "${GPUS[0]}"
) >"${OUTPUT}/logs/phase2_gpu${GPUS[0]}.log" 2>&1 & q0=$!
(
  run_current 01_time_uniform8 true_pose "${GPUS[1]}"
  run_recon 02_phone_spherical8 "${GPUS[1]}"
) >"${OUTPUT}/logs/phase2_gpu${GPUS[1]}.log" 2>&1 & q1=$!
(
  run_current 02_phone_spherical8 pose_mask "${GPUS[2]}"
  run_recon 01_time_uniform8 "${GPUS[2]}"
) >"${OUTPUT}/logs/phase2_gpu${GPUS[2]}.log" 2>&1 & q2=$!
(
  run_current 02_phone_spherical8 true_pose "${GPUS[3]}"
...
echo "OBJECTRON SHOE SS30K+SLAT30K MATRIX COMPLETE: ${OUTPUT}"
