...
STATE=${RUN}/logs/eval_dev64_step${STEP_PAD}_8gpu.state
EXIT_CODE=${RUN}/logs/eval_dev64_step${STEP_PAD}_8gpu.exit_code
LOCK=${RUN}/logs/eval_dev64_step${STEP_PAD}_8gpu.lock
...
exec 9>"${LOCK}"
if ! flock -n 9; then echo "Objaverse2K dev64 eight-GPU evaluation is already running" >&2; exit 99; fi
exec 8>"${TRAIN_LOCK}"
if ! flock -n 8; then echo "eight-GPU training or evaluation is still using the GPUs" >&2; exit 99; fi
finish() {
  RC=$?
  trap - EXIT
  printf '%s\n' "${RC}" >"${EXIT_CODE}"
