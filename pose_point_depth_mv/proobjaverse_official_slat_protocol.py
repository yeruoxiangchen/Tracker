...
COMBINED_SELECTION_FORMAT = "reconviagen.proobjaverse_paired_combined.v1"
...
COMBINED_30K_PAIR_COUNT = 30000
COMBINED_30K_TRAIN_COUNT = 29872
...
def _selection_canonical_sha256(value: Any) -> str:
    """Match the canonical identity used by the frozen selection producers."""
...
def _validate_selected_row(row: Any) -> dict[str, Any]:
    ...
    if not uid or not shard.startswith("shard-"):
        raise ValueError(f"selection row has invalid UID/shard: {row!r}")
    if not isinstance(render, dict) or not isinstance(slat, dict):
        raise ValueError(f"selection row lacks render/slat identity: uid={uid}")
    ...
    if render.get("path") != expected_render or slat.get("path") != expected_slat:
        raise ValueError(f"selection UID/path binding differs for uid={uid}")
    if int(render.get("size", 0)) <= 0 or int(slat.get("size", 0)) <= 0:
        raise ValueError(f"selection contains non-positive payload size for uid={uid}")
    return row


def validate_frozen_selection(selection: dict[str, Any]) -> dict[str, Any]:
    """Validate either frozen selection format without mixing identities."""

    selection_format = selection.get("format")
    if selection_format == SELECTION_FORMAT:
        forbidden = ("combined_selection_sha256", "repository", "pair_count")
        mixed = [key for key in forbidden if key in selection]
        if mixed:
            raise ValueError(
                "paired_subset.v1 contains paired_combined.v1 identity fields: "
                f"{mixed}"
            )
        body = {
            key: value
        ...
        expected_identity = _selection_canonical_sha256(body)
        if selection.get("selection_sha256") != expected_identity:
            raise ValueError("paired_subset.v1 selection identity hash is invalid")
        repo_id = selection.get("repo_id")
        revision = selection.get("revision")
        declared_count = int(selection.get("selected_pair_count", -1))
        identity_field = "selection_sha256"
    elif selection_format == COMBINED_SELECTION_FORMAT:
        forbidden = ("selection_sha256", "repo_id", "revision", "selected_pair_count")
        mixed = [key for key in forbidden if key in selection]
        if mixed:
            raise ValueError(
                "paired_combined.v1 contains paired_subset.v1 identity fields: "
                f"{mixed}"
            )
        body = {
            key: value
            for key, value in selection.items()
            if key != "combined_selection_sha256"
        }
        expected_identity = _selection_canonical_sha256(body)
        if selection.get("combined_selection_sha256") != expected_identity:
            raise ValueError("paired_combined.v1 selection identity hash is invalid")
        repository = selection.get("repository")
        if not isinstance(repository, dict) or set(repository) != {
            "repo_id",
            "revision",
        }:
            raise ValueError("paired_combined.v1 repository identity is invalid")
        repo_id = repository.get("repo_id")
        revision = repository.get("revision")
        declared_count = int(selection.get("pair_count", -1))
        identity_field = "combined_selection_sha256"
    else:
        raise ValueError(f"unexpected selection format={selection_format!r}")

    if not isinstance(repo_id, str) or not repo_id:
        raise ValueError("selection repository.repo_id is empty")
    if not isinstance(revision, str) or not revision:
        raise ValueError("selection repository.revision is empty")
    selected = selection.get("selected")
    if not isinstance(selected, list) or not selected:
        raise ValueError("selection contains no selected rows")
    rows = [_validate_selected_row(row) for row in selected]
    uids = [str(row["uid"]) for row in rows]
    if len(rows) != declared_count or len(uids) != len(set(uids)):
        raise ValueError(
            "selection UID accounting is invalid: "
            f"declared={declared_count} rows={len(rows)} unique={len(set(uids))}"
        )
    return {
        "format": selection_format,
        "identity_field": identity_field,
        "identity": expected_identity,
        "repository": {"repo_id": repo_id, "revision": revision},
        "rows": rows,
    }


def partition_rows(
    pool: list[dict[str, Any]], requested_splits: dict[str, int], *, seed: int
) -> dict[str, list[dict[str, Any]]]:
    """Apply the frozen deterministic split policy to an exact UID pool."""

    if len(pool) != sum(int(value) for value in requested_splits.values()):
        raise ValueError("pool size does not exactly cover the requested split contract")
    uids = [str(row.get("uid", "")) for row in pool]
    if not all(uids) or len(uids) != len(set(uids)):
        raise ValueError("protocol split pool contains empty/duplicate UIDs")
    ordered = sorted(
        pool,
        key=lambda row: hashlib.sha256(
            f"{int(seed)}:{row['uid']}".encode("ascii")
        ).hexdigest(),
    )
    splits: dict[str, list[dict[str, Any]]] = {}
    cursor = 0
    for name in ("decoder_audit", "predicted_support_bridge", "dev", "train"):
        count = int(requested_splits[name])
        splits[name] = ordered[cursor : cursor + count]
        cursor += count
    if cursor != len(pool):
        raise RuntimeError("protocol split cursor differs from the exact pool")
    return splits
...
def freeze_protocol(args: argparse.Namespace) -> None:
    ...
    if selection_binding["format"] == COMBINED_SELECTION_FORMAT and (
        int(args.pair_count) != COMBINED_30K_PAIR_COUNT
        or int(args.train_count) != COMBINED_30K_TRAIN_COUNT
    ):
        raise ValueError(
            "paired_combined.v1 is frozen for the exact official 30K contract: "
            f"pair_count={COMBINED_30K_PAIR_COUNT} "
            f"train_count={COMBINED_30K_TRAIN_COUNT}"
        )
    ...
    # while pool membership remains bound to the frozen selection order.
    splits = partition_rows(pool, requested_splits, seed=int(args.seed))
    ...
    protocol_body[str(selection_binding["identity_field"])] = str(
        selection_binding["identity"]
    )
    protocol_body.update(
        {
            "repository": dict(selection_binding["repository"]),
            "pair_count": len(pool),
            "seed": int(args.seed),
