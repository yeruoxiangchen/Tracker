...
``Stable-X/trellis-vggt-v0-2``.  It computes metrics in memory and, by default,
does not save predicted meshes or rendered previews.  ``--save_meshes`` is an
explicit diagnostic/export option; it does not alter inference or metrics.  The
aggregate command also supports a paired comparison with the existing official
Native-SS + trained-SLat endpoint reports.
"""
...
def _load_contract(args: argparse.Namespace) -> dict[str, Any]:
    ...
    paired_all = bool(getattr(args, "paired_targets_cover_all_objects", False))
    target_report_path = (
        None
        if paired_all
        else Path(args.target_report).expanduser().resolve(strict=True)
    )
    target_root = (
        None
        if paired_all
        else Path(args.target_mesh_root).expanduser().resolve(strict=True)
    )
    target_cache_roots = [
    ...
    split_manifest_sha256 = str(split_body.pop("manifest_sha256", ""))
    if (
        not split_manifest_sha256
        or canonical_sha256(split_body) != split_manifest_sha256
    ):
        raise RuntimeError("dev_split manifest identity differs")
    cache_report = load_json(cache_report_path)
    if paired_all and not target_cache_roots:
        raise RuntimeError(
            "paired_targets_cover_all_objects requires paired target cache roots"
        )
    target_report = None if paired_all else load_json(target_report_path)
    ...
    if str(split.get("protocol_sha256", "")) != str(
        cache_report.get("protocol_sha256", "")
    ):
        raise RuntimeError("dev split/cache official protocol identity differs")
    if len(cache_report.get("records", [])) != 64:
        raise RuntimeError("official Dev64 cache report does not contain 64 records")
    target_config = {} if target_report is None else dict(target_report.get("run_config", {}))
    if target_report is not None and (
    ...
    target_uids = (
        ...
        else [str(value) for value in target_config.get("object_uids", [])]
    )
    ...
    for index, (split_row, cache_row) in enumerate(zip(split_rows, cache_rows)):
        ...
        target_mesh = (
            ...
            else target_root / uid / "decoded_official_target.obj"
        )
        target_source = "official_target_obj_from_frozen_stock_decoder"
        if (paired_all or index >= 16) and target_cache_roots:
        ...
        if target_mesh is None or not target_mesh.is_file():
    ...
    return {
        ...
        "target_report_sha256": (
            "" if target_report_path is None else sha256_file(target_report_path)
        ),
            ...
            json.dumps(
                {
                    ...
                    "predicted_meshes_saved": bool(args.save_meshes),
            ...
            for seed in pending:
                ...
                record_common = {
                    **expected_by_seed[seed],
                    ...
                    "predicted_mesh_saved": bool(args.save_meshes),
                ...
                try:
                    ...
                    mesh_export = {}
                    if args.save_meshes:
                        # One object has multiple registered seeds.  Keep each
                        # exported Mesh in its own seed directory; a shared
                        # ``records/<uid>/mesh.obj`` would be overwritten by
                        # the next seed while the prior JSON retained its old
                        # content hash.
                        mesh_path = (
                            _record_path(output, row["uid"], seed).parent
                            / f"seed_{int(seed)}"
                            / "mesh.obj"
                        )
                        mesh_path.parent.mkdir(parents=True, exist_ok=True)
                        temporary = mesh_path.with_name(
                            f".{mesh_path.name}.tmp-{os.getpid()}"
                        )
                        predicted_mesh.export(temporary, file_type="obj")
...
def add_common_paths(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument("--target_report", default="")
    parser.add_argument("--target_mesh_root", default="")
    parser.add_argument(
        "--paired_target_cache_roots",
        default="",
        help=(
            "optional comma-separated target_mesh_cache directories from the "
            "current endpoint workers; legacy mode applies them to Dev[16:64), "
            "while --paired_targets_cover_all_objects applies them to all Dev64"
        ),
    )
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "use exact target NPZ files from paired_target_cache_roots for all "
            "Dev64 objects; target_report/target_mesh_root are then unnecessary"
        ),
    )
...
def make_parser() -> argparse.ArgumentParser:
    ...
    worker.add_argument(
        ...
        action="store_true",
        help=(
            "export each decoded prediction as mesh.obj in addition to the "
            "unchanged quantitative record"
        ),
    )
