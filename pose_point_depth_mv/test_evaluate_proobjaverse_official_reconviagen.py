...
class FullDevPairedTargetContractTest(unittest.TestCase):
    def _fixture(self, root: Path, *, cache_protocol: str = "protocol"):
        ...
        for index in range(64):
            uid = f"uid-{index:02d}"
            render = render_root / f"{uid}.tar"
            render.write_bytes(b"rgba")
            (target_root / f"{uid}.npz").write_bytes(b"target")
            split_rows.append(
                {
                    "uid": uid,
            ...
            cache_rows.append(
                {"uid": uid, "selected_view_ids": list(range(8))}
            )

        split = {
            "format": mod.SPLIT_FORMAT,
            "name": "dev",
            "count": 64,
        ...
        cache = {
            ...
            "vggt_executed": False,
            "protocol_sha256": cache_protocol,
            "records": cache_rows,
        }
        cache_path = root / "cache_report.json"
        cache_path.write_text(json.dumps(cache), encoding="utf-8")
        args = SimpleNamespace(
            dev_split=str(split_path),
            cache_report=str(cache_path),
            target_report="",
            target_mesh_root="",
            paired_target_cache_roots=str(target_root),
            paired_targets_cover_all_objects=True,
        )
        return args, target_root

    def test_all_dev_objects_use_exact_paired_targets(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            args, target_root = self._fixture(Path(directory))
            contract = mod._load_contract(args)
        self.assertEqual(len(contract["rows"]), 64)
        self.assertTrue(
            all(
                row["target_source"]
                == "exact_target_npz_used_by_current_endpoint_worker"
                for row in contract["rows"]
            )
        )
        self.assertEqual(contract["paired_target_cache_roots"], [str(target_root)])
        self.assertIsNone(contract["target_report_path"])

    def test_split_cache_protocol_mismatch_fails(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            args, _ = self._fixture(
                Path(directory), cache_protocol="different-protocol"
            )
            with self.assertRaisesRegex(RuntimeError, "protocol identity"):
                mod._load_contract(args)
