...
class SourceAssetIncompatibleError(ValueError):
    """A source mesh is inspectable but not compatible with the render frame."""
...
def build_code_bindings(args: argparse.Namespace) -> dict:
    paths = {
        "dataset_builder": Path(__file__).resolve(),
        "blender_render_script": BLENDER_RENDER_SCRIPT.resolve(),
    }
    if args.renderer == "blender":
        paths["blender_binary"] = Path(find_blender_binary(args)).resolve()
    return {
        name: {
            "path": str(path),
...
def vertex_colors_from_visual(mesh: trimesh.Trimesh) -> np.ndarray:
    ...
    if getattr(visual, "kind", None) == "vertex":
        vertex_colors = np.asarray(getattr(visual, "vertex_colors", None))
        if (
            vertex_colors.ndim == 2
            and vertex_colors.shape[0] == vertex_count
...
def validate_blender_bounds(
    metadata: dict,
    expected_vertices_render: np.ndarray,
    tolerance: float,
) -> dict:
    actual_bounds = metadata.get("normalized_scene_bounds")
    if not isinstance(actual_bounds, dict):
        raise RuntimeError(
            "Blender render metadata lacks normalized_scene_bounds"
        )
    actual_minimum = np.asarray(actual_bounds.get("minimum"), dtype=np.float32)
    actual_maximum = np.asarray(actual_bounds.get("maximum"), dtype=np.float32)
    expected_vertices = np.asarray(expected_vertices_render, dtype=np.float32)
    if (
        expected_vertices.ndim != 2
        or expected_vertices.shape[0] == 0
        or expected_vertices.shape[1] != 3
        or not np.all(np.isfinite(expected_vertices))
    ):
        raise RuntimeError("expected Blender bounds vertices are invalid")
    expected_minimum = expected_vertices.min(axis=0).astype(np.float32)
    expected_maximum = expected_vertices.max(axis=0).astype(np.float32)
    if (
        actual_minimum.shape != (3,)
        or actual_maximum.shape != (3,)
        or not np.all(np.isfinite(actual_minimum))
        or not np.all(np.isfinite(actual_maximum))
    ):
        raise RuntimeError(
            "Blender render metadata contains invalid normalized scene bounds"
        )
    bounds_max_abs = float(
        max(
            np.max(np.abs(actual_minimum - expected_minimum)),
            np.max(np.abs(actual_maximum - expected_maximum)),
        )
    )
    bounds_audit = {
        "normalized_scene_bounds_max_abs": bounds_max_abs,
        "normalized_scene_bounds_tolerance": float(tolerance),
        "expected_minimum": expected_minimum.astype(float).tolist(),
        "expected_maximum": expected_maximum.astype(float).tolist(),
        "actual_minimum": actual_minimum.astype(float).tolist(),
        "actual_maximum": actual_maximum.astype(float).tolist(),
        "source_suffix": metadata.get("source_suffix"),
        "source_import_policy": metadata.get("source_import_policy"),
        "normalization_policy": metadata.get("normalization_policy"),
    }
    if bounds_max_abs > float(tolerance):
        raise SourceAssetIncompatibleError(
            "source asset bounds incompatible between Blender and trimesh: "
            f"max_abs={bounds_max_abs:.8f}, "
            f"tolerance={float(tolerance):.8f}, "
            f"expected_min={expected_minimum.tolist()}, "
            f"expected_max={expected_maximum.tolist()}, "
            f"actual_min={actual_minimum.tolist()}, "
            f"actual_max={actual_maximum.tolist()}"
        )
    return bounds_audit
...
def render_mesh_blender(
    ...
    blender_env: dict[str, str] | None = None
    if args.xvfb_run_path:
        ...
        command_prefix = [
            ...
            (
                ...
                "-nolisten unix -listen local"
            ),
        ]
        # A rootless Debian-package extraction puts xvfb-run and Xvfb beside
        # each other.  xvfb-run resolves the server by name, so make that
        # sibling directory discoverable without modifying the login PATH.
        blender_env = dict(os.environ)
        blender_env["PATH"] = (
            f"{xvfb_run.parent}{os.pathsep}{blender_env.get('PATH', '')}"
        )
    with tempfile.TemporaryDirectory(prefix="pixal3d_mv_blender_") as tmpdir:
        tmpdir_path = Path(tmpdir)
        metadata_path = tmpdir_path / "render_metadata.json"
        expected_minimum = (
            np.asarray(expected_vertices_render, dtype=np.float32)
            .min(axis=0)
            .astype(float)
            .tolist()
        )
        ...
        request = {
            ...
            "bounds_tolerance": float(args.blender_bounds_tolerance),
            ...
            "cycles_device": args.blender_cycles_device,
            "world_strength": float(args.blender_world_strength),
            "light_energy": float(args.blender_light_energy),
        }
        # The historical builders pass one shared 3x3 intrinsic.  Registered
        # benchmark trajectories may vary FOV per view and therefore pass a
        # Vx3x3 array.  The Blender entrypoint supports both shapes; preserving
        # this value verbatim keeps the legacy path byte-for-byte unchanged.
        ...
        if args.blender_quiet:
            ...
            completed = subprocess.run(cmd, check=False, env=blender_env)

        metadata = None
        bounds_audit = None
        if metadata_path.is_file():
            metadata = json.loads(metadata_path.read_text(encoding="utf-8"))
            # This is intentionally evaluated before the process return code.
            # The Blender entrypoint writes and checks these bounds before
            # rendering, so a source-frame mismatch remains an asset rejection
            # instead of being mislabeled as an infrastructure failure.
            bounds_audit = validate_blender_bounds(
                metadata,
                expected_vertices_render,
                float(args.blender_bounds_tolerance),
            )
        if completed.returncode != 0:
            ...
            output_tail = (
                ...
                if output_lines
                else "(Blender output was streamed to the terminal)"
            )
            raise RuntimeError(
                "Blender render failed "
                f"(exit_code={completed.returncode}, command={cmd!r}). "
                "Last output lines:\n"
                f"{output_tail}"
            )
        if metadata is None or bounds_audit is None:
            raise RuntimeError("Blender render did not write render_metadata.json")
...
def circular_azimuth_stats(angles_degrees: Sequence[float]) -> dict:
    values = np.sort(np.asarray(angles_degrees, dtype=np.float64) % 360.0)
    if values.size < 2:
        return {
            "maximum_azimuth_gap_degrees": 360.0,
            "azimuth_coverage_degrees": 0.0,
    ...
    return {
        ...
        "azimuth_coverage_degrees": float(360.0 - maximum_gap),
        "azimuth_gap_std_degrees": float(np.std(gaps)),
    }


def camera_object_azimuth_degrees(c2w: np.ndarray) -> np.ndarray:
    matrices = np.asarray(c2w, dtype=np.float64)
    if matrices.ndim != 3 or matrices.shape[1:] != (4, 4):
        raise ValueError("camera azimuth expects c2w matrices with shape [N,4,4]")
    centers = matrices[:, :3, 3]
    radii_xz = np.linalg.norm(centers[:, [0, 2]], axis=1)
    if np.any(radii_xz <= 1.0e-8):
        raise ValueError("camera azimuth is undefined at the object center")
    return np.degrees(np.arctan2(centers[:, 0], centers[:, 2])) % 360.0


def object_azimuth_balanced_indices(
    good_indices: Sequence[int],
    c2w: np.ndarray,
    quality_scores: np.ndarray,
    count: int,
) -> tuple[list[int], dict]:
    """Select a deterministic subset that minimizes the largest circular gap."""

    valid = sorted({int(index) for index in good_indices})
    requested = int(count)
    if requested <= 0 or requested > len(valid):
        raise ValueError(
            f"invalid azimuth selection count={requested} for valid={len(valid)}"
        )
    angles = camera_object_azimuth_degrees(c2w)
    ...
    for start in valid:
        ...
        while len(chosen) < requested:
            ...
            next_index = max(
                ...
                key=lambda index: (
                    min(
                        abs(
                            (float(angles[index]) - float(angles[other]) + 180.0)
                            % 360.0
                            - 180.0
                        )
                        for other in chosen
        ...
        candidates.setdefault(key, tuple(chosen))

    def subset_score(indices: Sequence[int]) -> tuple:
        stats = circular_azimuth_stats(angles[list(indices)])
        quality = -sum(float(quality_scores[index]) for index in indices)
        return (
            round(float(stats["maximum_azimuth_gap_degrees"]), 10),
            round(float(stats["azimuth_gap_std_degrees"]), 10),
            quality,
            tuple(sorted(int(index) for index in indices)),
        )

    selected = list(min(candidates.values(), key=subset_score))
    selected.sort()
    stats = circular_azimuth_stats(angles[selected])
    return selected, {
        "search": "deterministic_multistart_farthest_object_azimuth",
        "candidate_subset_count": int(len(candidates)),
        "candidate_azimuth_degrees": [float(value) for value in angles.tolist()],
        "selected_azimuth_degrees": [float(angles[index]) for index in selected],
        **stats,
    }
...
def select_multiview_frames(
    ...
    if len(good_indices) < min_good:
        min_fg = int(
            _effective_selection_threshold(
                args.selection_min_fg_pixels_per_view,
                args.min_fg_pixels_per_view,
            )
        )
        min_area = float(
            _effective_selection_threshold(
                args.selection_min_fg_area_ratio,
                args.min_fg_area_ratio,
            )
        )
        min_margin = float(
            _effective_selection_threshold(
                args.selection_min_bbox_margin_px,
                args.min_bbox_margin_px,
            )
        )
        max_bbox_area = float(
            _effective_selection_threshold(
                args.selection_max_bbox_area_ratio,
                args.max_bbox_area_ratio,
            )
        )
        criterion_counts = {
            "fg": sum(stat["fg_pixels"] >= min_fg for stat in frame_stats),
            "area": sum(stat["fg_area_ratio"] >= min_area for stat in frame_stats),
            "margin": sum(stat["bbox_margin_px"] >= min_margin for stat in frame_stats),
            "bbox": sum(
                max_bbox_area <= 0
                or stat["bbox_area_ratio"] <= max_bbox_area
                for stat in frame_stats
            ),
        }
        fg_values = [int(stat["fg_pixels"]) for stat in frame_stats]
        area_values = [float(stat["fg_area_ratio"]) for stat in frame_stats]
        margin_values = [int(stat["bbox_margin_px"]) for stat in frame_stats]
        bbox_values = [float(stat["bbox_area_ratio"]) for stat in frame_stats]
        raise ValueError(
            f"frame selection rejected: good_candidate_views={len(good_indices)} < {min_good}, "
            f"candidate_views={candidate_count}, criterion_counts={criterion_counts}, "
            f"thresholds=(fg>={min_fg}, area>={min_area:.6f}, "
            f"margin>={min_margin:.1f}, bbox<={max_bbox_area:.6f}), "
            f"ranges=(fg={min(fg_values)}..{max(fg_values)}, "
            f"area={min(area_values):.6f}..{max(area_values):.6f}, "
            f"margin={min(margin_values)}..{max(margin_values)}, "
            f"bbox={min(bbox_values):.6f}..{max(bbox_values):.6f})"
        )
    ...
    elif args.frame_selection_policy == "object_azimuth_balanced":
        selected, azimuth_stats = object_azimuth_balanced_indices(
            good_indices,
            c2w,
            quality_scores,
    ...
    if azimuth_stats is None:
        angles = camera_object_azimuth_degrees(c2w)
        azimuth_stats = {
            "search": "selection_policy_not_azimuth_optimized",
            "candidate_subset_count": None,
            "candidate_azimuth_degrees": [float(value) for value in angles.tolist()],
            "selected_azimuth_degrees": [float(angles[index]) for index in selected],
            **circular_azimuth_stats(angles[selected]),
        }
    coverage = float(azimuth_stats["azimuth_coverage_degrees"])
    maximum_gap = float(azimuth_stats["maximum_azimuth_gap_degrees"])
    if coverage + 1.0e-6 < float(args.min_selected_azimuth_coverage):
        raise ValueError(
            "frame selection rejected: selected object-azimuth coverage "
            f"{coverage:.3f} < {float(args.min_selected_azimuth_coverage):.3f} degrees"
        )
    if maximum_gap - 1.0e-6 > float(args.max_selected_azimuth_gap):
        raise ValueError(
            "frame selection rejected: selected maximum object-azimuth gap "
            f"{maximum_gap:.3f} > {float(args.max_selected_azimuth_gap):.3f} degrees"
        )
    return selected, {
        ...
        "azimuth_contract": {
            "min_selected_azimuth_coverage_degrees": float(
                args.min_selected_azimuth_coverage
            ),
            "max_selected_azimuth_gap_degrees": float(
                args.max_selected_azimuth_gap
            ),
        },
        **azimuth_stats,
    }
...
def classify_failure(error: str) -> str:
    lowered = error.lower()
    if "source asset bounds incompatible" in lowered:
...
def build_sample(
    ...
    if args.renderer == "blender":
        images_and_masks, renderer_audit = render_mesh_blender(
            glb_path,
            center,
            scale,
            c2w,
        ...
        renderer_audit = {
            "normalized_scene_bounds_max_abs": None,
            "normalized_scene_bounds_tolerance": None,
        }
        images_and_masks = []
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--min_selected_azimuth_coverage", type=float, default=0.0)
    parser.add_argument("--max_selected_azimuth_gap", type=float, default=360.0)
    ...
    parser.add_argument(
        ...
        default=1.0e-3,
        help=(
            "Maximum absolute difference between normalized Blender and "
            "trimesh AABBs. Larger differences reject only the source asset, "
            "rather than marking the renderer infrastructure failed."
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Optional xvfb-run executable used to provide the X display required "
            "by Blender EEVEE on headless Linux nodes."
        ),
    )
...
def semantic_build_config(args: argparse.Namespace) -> dict:
    """Return sample-affecting arguments for cross-shard provenance checks.

    Paths and shard slicing controls intentionally stay outside this binding.
    Every rendering, trajectory, quality, and latent-encoding option remains
    bound so independently produced shards cannot be silently mixed.
    """
...
def summarize_renderer_audits(samples: list[dict]) -> dict:
    values = [
        float(sample["renderer_audit"]["normalized_scene_bounds_max_abs"])
        for sample in samples
        if sample.get("renderer_audit", {}).get(
...
def main() -> None:
    ...
    if not 0.0 <= float(args.min_selected_azimuth_coverage) <= 360.0:
        raise ValueError("min_selected_azimuth_coverage must be in [0,360]")
    if not 0.0 <= float(args.max_selected_azimuth_gap) <= 360.0:
        raise ValueError("max_selected_azimuth_gap must be in [0,360]")
    if (
    ...
    code_bindings = build_code_bindings(args)
    ...
    renderer_audit_summary = summarize_renderer_audits(samples)
    common = {
        ...
        "renderer": args.renderer,
        "build_config": semantic_build_config(args),
        ...
        "quality_policy": {
            ...
            "min_selected_azimuth_coverage": args.min_selected_azimuth_coverage,
            "max_selected_azimuth_gap": args.max_selected_azimuth_gap,
        ...
        "renderer_audit_summary": renderer_audit_summary,
        "samples": samples,
        "failures": failures,
    }
