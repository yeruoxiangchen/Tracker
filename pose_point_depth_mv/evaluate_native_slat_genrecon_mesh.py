#!/usr/bin/env python3
"""Same-Native-SS-coordinate/same-noise Mesh evaluation for Native-SLAT v2."""
...
REPORT_VERSION = "pose_point_depth_mv.native_slat_genrecon_mesh.v2"
...
def main() -> None:
    ...
    if dataset.config.get("condition_arch") != "native_ss_genrecon_v2":
        raise RuntimeError("Native-SLAT v2 Mesh evaluation requires its v2 cache")
    ...
    for object_position, (object_uid, uid, seed_map) in enumerate(selections):
        for seed in seeds:
            ...
            print(f"[native_slat_v2_mesh] {pair_id} {uid}", flush=True)
    ...
    report = {
        ...
        "records": records,
        "scope_guard": (
            "development comparison of Native-SLAT v2 against the frozen "
    ...
    lines = [
        "Native-SLAT v2 same-Native-SS-coordinate Mesh evaluation",
        "=" * 61,
