...
class TrellisVGGTTo3DPipeline(TrellisImageTo3DPipeline):
    ...
    def run(
        ...
        preprocess_image: bool = True,
        mode: Literal['stochastic', 'multidiffusion'] = 'stochastic',
        return_ss_support_only: bool = False,
    ):
        ...
        # Keep pure sparse-structure evaluation on the exact same execution
        # path as the original full ReconViaGen pipeline.  Returning here is
        # the only difference: no SLat condition, SLat flow, or representation
        # decoder (Mesh/Gaussian/RF) is run.
        if return_ss_support_only:
            return None, coords, ss_noise
