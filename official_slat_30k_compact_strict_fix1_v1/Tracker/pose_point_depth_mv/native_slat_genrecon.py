...
def validate_strict_cpu_lifting_sample(sample: dict[str, Any]) -> dict[str, int]:
    """Enforce the strict CPU-select-before-H2D projection input contract.
    ...
    required = ("visual_patch_features", "intrinsics", "extrinsics")
    ...
    if non_cpu:
        raise RuntimeError(
            "strict lifting CPU-selection contract violated before view "
    ...
    if "image_size" in sample:
        ...
        if (
            ...
            raise ValueError("compact lifting image_size must be two positive integers")
    elif not torch.is_tensor(sample.get("predicted_depth")):
        raise ValueError(
            "Native-SLAT lifting sample requires compact image_size metadata or "
            "legacy predicted_depth"
        )
    return {
...
def select_sparse_frustum_inputs_cpu(
    ...
    if "image_size" in sample:
        image_shape = tuple(map(int, sample["image_size"]))
    else:
