#!/usr/bin/env python3
"""Same-coordinate/same-noise Mesh evaluation for condition-only Native-SLat."""
...
REPORT_VERSION = "pose_point_depth_mv.native_slat_condition_only_mesh.v1"
...
def main() -> None:
    ...
    if dataset.config.get("condition_arch") != "native_ss_genrecon_v2":
        raise RuntimeError("condition-only Mesh evaluation requires the v2 cache")
    ...
    normalization_bindings = resolve_native_objaverse_normalization_bindings(
        args.cache_manifest,
        json.loads(Path(args.cache_manifest).read_text(encoding="utf-8")),
        [dataset.rows[seed_map[seeds[0]]] for _, _, seed_map in selections],
    )
    ...
    if decoder is None:
        raise RuntimeError("condition-only Mesh evaluation requires frozen decoder")
    ...
    run_config = {
        ...
        "architecture_guard": {
            "slat_attention_lora": False,
            "trainable_context_view_fusion": False,
            "trainable_modules": ["aggregator", "block_condition"],
        },
    }
    ...
    for object_position, (object_uid, uid, seed_map) in enumerate(selections):
        for seed in seeds:
            ...
            for branch in ("stock", "full"):
                ...
                if branch == "stock":
                    flow = NativeSLatConditionOnlyStockFlow(model)
                else:
                    flow = NativeSLatConditionOnlyCFGFlow(
                        model,
                        condition["cond"],
                ...
                if branch == "full":
                    ...
                    if flow.positive_calls <= 0 or flow.negative_calls <= 0:
                        raise RuntimeError("condition-only standard CFG missed a branch")
            ...
            print(f"[native_slat_condition_only_mesh] {pair_id} {uid}", flush=True)
    ...
    report = {
        ...
        "records": records,
        "scope_guard": (
            "development comparison of LoRA-free posed-DINO condition-only Native-SLat "
            "against frozen Native-SS+Stock-SLat; architecture/checkpoint selection only"
        ),
    }
    ...
    lines = [
        "Native-SLat condition-only same-coordinate Mesh evaluation",
        "=" * 62,
