#!/usr/bin/env python3
"""Frozen Native-SLAT v2 compatibility entrypoint.

This module preserves the released v2 checkpoint schema after the main
``native_slat_genrecon`` implementation moved to v3 learned view fusion.  It
does not upgrade, pad, or reinterpret v2 trainable state as v3 state.
"""
...
NATIVE_SLAT_GENRECON_V2_VERSION = "pose_point_depth_mv.native_slat_genrecon.v2"
NATIVE_SLAT_GENRECON_V2_PROJECTION = (
    "active32_frustum_only_dino_shared_geometry_every_block.v2"
)


class NativeSLatGenreconV2Flow(nn.Module):
    """Exact v2 trainable surface: LoRA + aggregator + block condition."""
    ...
    def _adapted_core_forward(
        ...
        if int(condition_3d.shape[0]) != int(h.feats.shape[0]):
            raise ValueError("Native-SLAT v2 active condition/feature counts differ")
        # Calling the released block directly preserves its fixed arithmetic
        # mean over per-view cross-attention outputs.
    ...
    def adapted_prediction(
        ...
        *,
        ...
        prediction = self._adapted_core_forward(
            x, t, condition, condition_3d
        )
        if not torch.equal(prediction.coords, stock.coords):
            raise RuntimeError("Native-SLAT v2 adaptation changed sparse coordinates")
...
def build_native_slat_genrecon_v2_components(
    *,
    pretrained: str,
    ...
    need_decoder: bool,
    decoder_kind: str = "mesh",
    decoder_to_device: bool = True,
    device: torch.device,
) -> tuple[Any, NativeSLatGenreconV2Flow, Any | None, dict[str, Any], dict[str, Any], dict[str, Any]]:
    ...
    mesh_decoder = pipeline.models["slat_decoder_mesh"].eval()
    validate_runtime_stock_slat(
        stock_slat_freeze,
        pretrained=pretrained,
        flow=base_flow,
        decoder=mesh_decoder,
        sampler_params=dict(pipeline.slat_sampler_params),
        normalization=dict(pipeline.slat_normalization),
    )
    if decoder_kind == "mesh":
        decoder: Any = mesh_decoder
    elif decoder_kind == "gaussian":
        decoder = pipeline.models["slat_decoder_gs"].eval()
    elif decoder_kind == "mesh_and_gaussian":
        decoder = {
            "mesh": mesh_decoder,
            "gaussian": pipeline.models["slat_decoder_gs"].eval(),
        }
    else:
        raise ValueError(
            "decoder_kind must be mesh, gaussian, or mesh_and_gaussian"
        )
    base_flow = base_flow.to(device)
    if need_decoder and decoder_to_device:
        if isinstance(decoder, dict):
            decoder = {name: module.to(device) for name, module in decoder.items()}
        else:
            decoder = decoder.to(device)
    for parameter in base_flow.parameters():
        parameter.requires_grad_(False)
    decoder_modules = decoder.values() if isinstance(decoder, dict) else (decoder,)
    ...
    model = NativeSLatGenreconV2Flow(
    ...
    if not lora_modules or covered_blocks != list(range(block_count)):
        raise RuntimeError(
            f"Native-SLAT v2 LoRA coverage failed modules={len(lora_modules)} "
    ...
    if unexpected or not trainable_names:
        raise RuntimeError(f"Native-SLAT v2 trainable whitelist failed: {unexpected}")
    lora_parameters = sum(
        parameter.numel()
        for name, parameter in model.named_parameters()
...
def validate_native_slat_genrecon_v2_checkpoint(
    ...
    *,
    ...
    if checkpoint.get("format") != NATIVE_SLAT_GENRECON_V2_VERSION:
        raise ValueError(f"unexpected Native-SLAT v2 format={checkpoint.get('format')!r}")
    summary = checkpoint.get("model_summary")
    args = checkpoint.get("args")
    if not isinstance(summary, dict) or not isinstance(args, dict):
        raise ValueError("Native-SLAT v2 checkpoint lacks summary/arguments")
    required = {
        "pretrained": str(pretrained),
        "baseline": NATIVE_SLAT_BASELINE,
        "projection": NATIVE_SLAT_GENRECON_V2_PROJECTION,
    ...
    if "context_view_fusion" in summary:
        raise ValueError("Native-SLAT v2 checkpoint unexpectedly contains v3 view fusion")
    if summary.get("stock_slat_freeze", {}).get("freeze_sha256") != stock_slat_freeze.get(
        "freeze_sha256"
    ):
        raise RuntimeError("Native-SLAT v2 checkpoint Stock freeze differs")
    if summary.get("upstream_native_ss") != upstream_native_ss:
        raise RuntimeError("Native-SLAT v2 checkpoint upstream Native SS differs")
    forbidden = {
        "condition_scale",
        "delta_rms_ratio_cap",
        "delta_norm_weight",
    ...
    if present:
        raise ValueError(f"Native-SLAT v2 contains forbidden fields={present}")
    for key in ("model_trainable_state", "ema_trainable_state"):
        if not isinstance(checkpoint.get(key), dict):
            raise ValueError(f"Native-SLAT v2 checkpoint lacks {key}")
        if any(name.startswith("view_fusion.") for name in checkpoint[key]):
            raise ValueError(f"Native-SLAT v2 {key} contains v3 parameters")
    ...
    if (
        ...
        raise ValueError("Native-SLAT v2 checkpoint EMA contract mismatch")
...
def optimizer_parameter_groups(
    ...
    *,
    ...
    if not lora or not new:
        raise RuntimeError("Native-SLAT v2 optimizer requires LoRA and condition parameters")
    return [
