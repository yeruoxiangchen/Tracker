...
def main() -> None:
    ...
    if not protocol_sha256 or split not in {"train", "dev", "val", "holdout"}:
        raise RuntimeError("official SLat source lacks frozen protocol/split")
    domain = official_domain_contract(
        protocol_sha256=protocol_sha256,
        encoder_pretrained=str(args.ss_encoder_pretrained),
        decoder_pretrained=str(args.ss_decoder_pretrained),
    ...
    for position, (slat_row, lifting_row) in enumerate(joined, start=1):
        ...
        source_target = Path(slat_row["target_file"]).expanduser().resolve()
