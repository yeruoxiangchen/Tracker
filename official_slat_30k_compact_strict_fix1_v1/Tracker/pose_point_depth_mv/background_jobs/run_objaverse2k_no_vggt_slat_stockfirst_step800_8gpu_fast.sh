...
GPUS=${OBJAVERSE2K_STOCKFIRST_GPUS:-0,1,2,3,4,5,6,7}
NUM_WORKERS=${OBJAVERSE2K_STOCKFIRST_NUM_WORKERS:-2}
GRADIENT_CHECKPOINTING=${OBJAVERSE2K_STOCKFIRST_GRADIENT_CHECKPOINTING:-0}
...
OUT=${RUN}/slat_objaverse2135_stockinit_first_step2000_seed42_8gpu_bs8_fastio_v1
STATE=${RUN}/logs/slat_objaverse2135_stockfirst_step800_8gpu_fast.state
EXIT_CODE=${RUN}/logs/slat_objaverse2135_stockfirst_step800_8gpu_fast.exit_code
LOCK=${RUN}/logs/slat_objaverse2135_stockfirst_step800_8gpu_fast.lock
...
if [ "${#GPU_ARRAY[@]}" -ne 8 ] || \
  ...
  echo "OBJAVERSE2K_STOCKFIRST_GPUS requires eight distinct GPUs" >&2
  exit 96
fi
if ! [[ "${NUM_WORKERS}" =~ ^[1-9][0-9]*$ ]]; then
  echo "OBJAVERSE2K_STOCKFIRST_NUM_WORKERS must be a positive integer" >&2
  exit 96
fi
if [ "${GRADIENT_CHECKPOINTING}" != 0 ] && \
   [ "${GRADIENT_CHECKPOINTING}" != 1 ]; then
  echo "OBJAVERSE2K_STOCKFIRST_GRADIENT_CHECKPOINTING must be 0 or 1" >&2
  exit 96
fi
...
exec 9>"${LOCK}"
if ! flock -n 9; then
  echo "Objaverse2K Stock-first step800 eight-GPU training is already running" >&2
  exit 99
fi
...
trap finish EXIT
printf 'started_at=%s state=validating gpus=%s world_size=8 grad_accum=1 global_batch=8 run_until_step=%s initialization=stock stock_context=first num_workers=%s gradient_checkpointing=%s\n' \
  "$(date --iso-8601=seconds)" "${GPUS}" "${RUN_UNTIL_STEP}" \
  "${NUM_WORKERS}" "${GRADIENT_CHECKPOINTING}" >"${STATE}"
rm -f "${EXIT_CODE}"

# Validate every materialized cache artifact once before torchrun. Passing
# --verify_cache_hashes to torchrun would repeat the same disk scan in 8 ranks.
...
printf 'started_at=%s state=running gpus=%s world_size=8 grad_accum=1 global_batch=8 run_until_step=%s initialization=stock stock_context=first num_workers=%s gradient_checkpointing=%s\n' \
  "$(date --iso-8601=seconds)" "${GPUS}" "${RUN_UNTIL_STEP}" \
  "${NUM_WORKERS}" "${GRADIENT_CHECKPOINTING}" >"${STATE}"

if [ -s "${OUT}/checkpoints/step_000800.pt" ] && \
   [ -s "${OUT}/stage_report_step_000800.json" ]; then
  echo "Objaverse2K Stock-first step800 eight-GPU training already complete: ${OUT}"
else
  if [ -s "${OUT}/checkpoints/last.pt" ]; then
    ...
    echo "Stock-first output exists without resumable last.pt: ${OUT}" >&2
    exit 98
  else
...
"${PY}" - "${OUT}/checkpoints/step_000800.pt" \
  "${OUT}/stage_report_step_000800.json" "${OUT}" \
  "${NUM_WORKERS}" "${GRADIENT_CHECKPOINTING}" <<'PY'
import json
import sys
...
assert args["stock_context_views"] == "first"
...
print({
    ...
    "cache_hash_scan": "single_process",
