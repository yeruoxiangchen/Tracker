...
GPUS=${OBJAVERSE2K_SLAT_EVAL_GPUS:-0,1,2,3,4,5,6,7}
STEP=${OBJAVERSE2K_SLAT_EVAL_STEP:-800}
...
CANDIDATE=${RUN}/slat_objaverse2135_step2000_seed42_8gpu_bs8_v1/checkpoints/step_${STEP_PAD}.pt
OUT=${RUN}/eval_dev16_step${STEP_PAD}_stock_m8_objaverse2k_8gpu_v1
STATE=${RUN}/logs/eval_dev16_step${STEP_PAD}_8gpu.state
EXIT_CODE=${RUN}/logs/eval_dev16_step${STEP_PAD}_8gpu.exit_code
LOCK=${RUN}/logs/eval_dev16_step${STEP_PAD}_8gpu.lock
TRAIN_LOCK=${RUN}/logs/slat_objaverse2135_8gpu_bs8_training.lock

IFS=',' read -r -a GPU_ARRAY <<<"${GPUS}"
if [ "${#GPU_ARRAY[@]}" -ne 8 ] || \
   [ "$(printf '%s\n' "${GPU_ARRAY[@]}" | sort -u | wc -l)" -ne 8 ]; then
  echo "OBJAVERSE2K_SLAT_EVAL_GPUS requires eight distinct GPUs" >&2
  exit 96
fi
...
exec 9>"${LOCK}"
if ! flock -n 9; then echo "Objaverse2K dev16 evaluation is already running" >&2; exit 99; fi
exec 8>"${TRAIN_LOCK}"
if ! flock -n 8; then echo "eight-GPU Objaverse2K training is still running" >&2; exit 99; fi
finish() {
  RC=$?
  trap - EXIT
  printf '%s\n' "${RC}" >"${EXIT_CODE}"
...
trap finish EXIT
printf 'started_at=%s state=running gpus=%s step=%s objects=%s\n' \
  "$(date --iso-8601=seconds)" "${GPUS}" "${STEP}" "${EXPECTED_OBJECTS}" >"${STATE}"
rm -f "${EXIT_CODE}"

LABELS=(m8 m8 m8 m8 objaverse2k objaverse2k objaverse2k objaverse2k)
...
for SLOT in 0 1 2 3 4 5 6 7; do
  ...
  "${PY}" -u -m pose_point_depth_mv.evaluate_objaverse2k_no_vggt_slat worker \
    ...
    >"${RUN}/logs/eval_dev16_step${STEP_PAD}_${LABEL}_worker_${WORKER}.log" 2>&1 &
  pids+=("$!")
done
failed=0
for PID in "${pids[@]}"; do
...
if [ "${failed}" -ne 0 ]; then
  echo "one or more dev16 evaluation workers failed" >&2
  exit 2
fi

if [ ! -s "${OUT}/comparison/report.json" ]; then
  "${PY}" -u -m pose_point_depth_mv.evaluate_objaverse2k_no_vggt_slat aggregate \
    --m8_reports "${OUT}/m8_worker_0/report.json,${OUT}/m8_worker_1/report.json,${OUT}/m8_worker_2/report.json,${OUT}/m8_worker_3/report.json" \
    --objaverse2k_reports "${OUT}/objaverse2k_worker_0/report.json,${OUT}/objaverse2k_worker_1/report.json,${OUT}/objaverse2k_worker_2/report.json,${OUT}/objaverse2k_worker_3/report.json" \
