...
GPUS=${OBJAVERSE2K_SLAT_FOURWAY_GPUS:-0,1,2,3,4,5,6,7}
...
OUT=${RUN}/eval_fourway_dev16_step${STEP_PAD}_8gpu_v1
STATE=${RUN}/logs/eval_fourway_dev16_step${STEP_PAD}_8gpu.state
EXIT_CODE=${RUN}/logs/eval_fourway_dev16_step${STEP_PAD}_8gpu.exit_code
LOCK=${RUN}/logs/eval_fourway_dev16_step${STEP_PAD}_8gpu.lock
TRAIN_LOCK=${RUN}/logs/slat_objaverse2135_8gpu_bs8_training.lock

IFS=',' read -r -a GPU_ARRAY <<<"${GPUS}"
if [ "${#GPU_ARRAY[@]}" -ne 8 ] || \
   [ "$(printf '%s\n' "${GPU_ARRAY[@]}" | sort -u | wc -l)" -ne 8 ]; then
  echo "OBJAVERSE2K_SLAT_FOURWAY_GPUS requires eight distinct GPUs" >&2
  exit 96
fi
...
exec 9>"${LOCK}"
if ! flock -n 9; then
  echo "Objaverse2K four-way dev16 evaluation is already running" >&2
  exit 99
fi
exec 8>"${TRAIN_LOCK}"
if ! flock -n 8; then
  echo "eight-GPU Objaverse2K training or evaluation is still running" >&2
  exit 99
fi
finish() {
  RC=$?
...
for WORKER in 0 1 2 3 4 5 6 7; do
  ...
  "${PY}" -u -m pose_point_depth_mv.evaluate_objaverse2k_no_vggt_slat_fourway worker \
    ...
    >"${RUN}/logs/eval_fourway_dev16_step${STEP_PAD}_worker_${WORKER}.log" 2>&1 &
  pids+=("$!")
done
...
if [ "${failed}" -ne 0 ]; then
  echo "one or more four-way dev16 workers failed" >&2
  exit 2
fi
