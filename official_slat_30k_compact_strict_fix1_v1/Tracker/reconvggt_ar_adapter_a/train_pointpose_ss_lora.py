...
class PointPoseCacheDataset(Dataset):
    def __init__(self, manifest: str | Path, *, indices: str = "all") -> None:
        ...
        self.source_by_uid: dict[str, dict[str, Any]] = {}
        self.prior_payload: dict[str, Any] | None = None
        self.prior_by_uid: dict[str, dict[str, Any]] = {}
        source_manifest = payload.get("source_manifest")
        prior_manifest = payload.get("prior_manifest")
        if source_manifest is not None and prior_manifest is not None:
            ...
            if not prior_path.is_absolute():
                prior_path = self.manifest_path.parent / prior_path
            ...
            for label, rows, destination in (
                ("source", self.source_payload.get("samples"), self.source_by_uid),
                ("prior", self.prior_payload.get("samples"), self.prior_by_uid),
            ):
                if not isinstance(rows, list) or not rows:
                    raise ValueError(f"{label} manifest has no samples: {manifest}")
                for item in rows:
                    uid = str(item.get("uid", ""))
                    if not uid or uid in destination:
                        raise ValueError(f"{label} manifest contains empty/duplicate uid={uid!r}")
    ...
    def __getitem__(self, index: int) -> dict[str, Any]:
        ...
        with np.load(physical_path) as data:
            ...
            prior_conf = np.asarray(data["prior_conf"], dtype=np.float32)
            view_ids = np.asarray(data["view_ids"], dtype=np.int64)
        ...
        if prior_coords.ndim != 2 or prior_coords.shape[1] not in (3, 4):
            raise ValueError(f"uid={row['uid']} invalid prior_coords shape {prior_coords.shape}")
        if prior_conf.ndim != 1 or len(prior_conf) != len(prior_coords):
            raise ValueError(f"uid={row['uid']} invalid prior_conf shape {prior_conf.shape}")
        if view_ids.ndim != 1 or not len(view_ids):
            raise ValueError(f"uid={row['uid']} invalid view_ids shape {view_ids.shape}")
        if not np.isfinite(prior_conf).all():
            raise ValueError(f"uid={row['uid']} prior confidence contains non-finite values")
        ...
        if self.source_payload is not None and self.prior_payload is not None:
            uid = str(row["uid"])
            source = self.source_by_uid.get(uid)
            prior = self.prior_by_uid.get(uid)
            if source is None or prior is None:
                raise KeyError(f"uid={uid} missing from source/prior metadata")
            frames = source.get("frames") or []
            if (view_ids < 0).any() or (view_ids >= len(frames)).any():
                raise ValueError(f"uid={uid} view_ids outside source frame range")
            ...
            if intrinsics.shape != (len(view_ids), 3, 3):
                raise ValueError(f"uid={uid} invalid selected intrinsics {intrinsics.shape}")
            if extrinsics.shape != (len(view_ids), 4, 4):
                raise ValueError(f"uid={uid} invalid selected extrinsics {extrinsics.shape}")
            if len(result["image_paths"]) != len(view_ids) or len(result["mask_paths"]) != len(view_ids):
                raise ValueError(f"uid={uid} image/mask/view count mismatch")
            result.update(
                {
