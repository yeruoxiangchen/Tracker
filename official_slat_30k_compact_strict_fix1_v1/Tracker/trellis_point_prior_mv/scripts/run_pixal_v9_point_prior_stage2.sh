...
CONNECTIVITY_LOSS_WEIGHT=${CONNECTIVITY_LOSS_WEIGHT:-0.0}
CONNECTIVITY_NEIGHBOR_THRESHOLD=${CONNECTIVITY_NEIGHBOR_THRESHOLD:-0.08}
...
if [[ "$RUN_TRAIN" == "1" ]]; then
  ...
  env "${COMMON_ENV[@]}" "$PY" -u trellis_point_prior_mv/train_sparse_inpaint_stage2.py \
    ...
    --connectivity_loss_weight "$CONNECTIVITY_LOSS_WEIGHT" \
    --connectivity_neighbor_threshold "$CONNECTIVITY_NEIGHBOR_THRESHOLD" \
    --lora_rank "$LORA_RANK" \
